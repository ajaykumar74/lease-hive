...
@Component({
    templateUrl: './numberSequence-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class NumberSequenceViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    numberSequence: INumberSequence = {} as INumberSequence;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/numberSequence/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
