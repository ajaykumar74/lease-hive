...
@Component({
  ...
  standalone: false,
  templateUrl: './partyDocument-list.component.html'
})
export class PartyDocumentListComponent implements OnInit {
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.partyDocumentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.partyDocumentService.search(searchParam).subscribe({
        next: res => {
      ...
      this.SetListData(this.partyDocumentService.CacheData.Data, this.partyDocumentService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'DocumentNumber', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
