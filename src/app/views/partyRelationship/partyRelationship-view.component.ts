...
@Component({
    templateUrl: './partyRelationship-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class PartyRelationshipViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    partyRelationship: IPartyRelationship = {} as IPartyRelationship;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/partyRelationship/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
