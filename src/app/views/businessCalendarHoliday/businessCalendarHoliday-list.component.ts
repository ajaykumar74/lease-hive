...
@Component({
  ...
  standalone: false,
  templateUrl: './businessCalendarHoliday-list.component.html'
})
export class BusinessCalendarHolidayListComponent implements OnInit {
  ...
  lstMain: IBusinessCalendarHoliday[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.businessCalendarHolidayService.CacheData.IsLoaded) {
      this.currentPage = this.businessCalendarHolidayService.CacheData.CurrentPage;
      this.objSearch = this.businessCalendarHolidayService.CacheData.objSearch;
      this.permission = this.businessCalendarHolidayService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.businessCalendarHolidayService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.businessCalendarHolidayService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.businessCalendarHolidayService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.businessCalendarHolidayService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.businessCalendarHolidayService.CacheData.Data, this.businessCalendarHolidayService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'HolidayName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
