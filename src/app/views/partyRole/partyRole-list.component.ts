...
@Component({
  ...
  standalone: false,
  templateUrl: './partyRole-list.component.html'
})
export class PartyRoleListComponent implements OnInit {
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.partyRoleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.partyRoleService.search(searchParam).subscribe({
        next: res => {
      ...
      this.SetListData(this.partyRoleService.CacheData.Data, this.partyRoleService.CacheData.TotalRecords);
    }
  }
