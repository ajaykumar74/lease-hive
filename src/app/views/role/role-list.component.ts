...
export class RoleListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.roleService.CacheData.IsLoaded) {
      this.currentPage = this.roleService.CacheData.CurrentPage;
      this.objSearch = this.roleService.CacheData.objSearch;
      this.permission = this.roleService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.roleService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.roleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.roleService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.roleService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.roleService.CacheData.Data, this.roleService.CacheData.TotalRecords);
    }
  }
...
if (this.objSearch.AuditType == 'Created') {
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/roles/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['roles/view/' + obj.Id]);
    } 
