...
export class CustomerListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerService.CacheData.IsLoaded) {
      this.currentPage = this.customerService.CacheData.CurrentPage;
      this.objSearch = this.customerService.CacheData.objSearch;
      this.permission = this.customerService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.customerService.search(searchParam).subscribe({
        next: res => {
          ...
          this.customerService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerService.CacheData.Data, this.customerService.CacheData.TotalRecords);
    }
  }
