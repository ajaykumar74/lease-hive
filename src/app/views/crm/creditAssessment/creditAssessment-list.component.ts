...
@Component({
  ...
  standalone: false,
  templateUrl: './creditAssessment-list.component.html'
})
export class CreditAssessmentListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditAssessmentService.CacheData.IsLoaded) {
      this.currentPage = this.creditAssessmentService.CacheData.CurrentPage;
      this.objSearch = this.creditAssessmentService.CacheData.objSearch;
      this.permission = this.creditAssessmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditAssessmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditAssessmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditAssessmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditAssessmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditAssessmentService.CacheData.Data, this.creditAssessmentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['business/origination/credit/review/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['business/origination/credit/review/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['business/origination/credit/review/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
