...
@Component({
  ...
  standalone: false,
  templateUrl: './quotePartySnapshot-list.component.html'
})
export class QuotePartySnapshotListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.quotePartySnapshotService.CacheData.IsLoaded) {
      this.currentPage = this.quotePartySnapshotService.CacheData.CurrentPage;
      this.objSearch = this.quotePartySnapshotService.CacheData.objSearch;
      this.permission = this.quotePartySnapshotService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.quotePartySnapshotService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.quotePartySnapshotService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.quotePartySnapshotService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.quotePartySnapshotService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.quotePartySnapshotService.CacheData.Data, this.quotePartySnapshotService.CacheData.TotalRecords);
    }
  }
