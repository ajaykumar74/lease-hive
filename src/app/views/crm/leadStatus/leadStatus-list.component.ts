...
export class LeadStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leadStatusService.CacheData.IsLoaded) {
      this.currentPage = this.leadStatusService.CacheData.CurrentPage;
      this.objSearch = this.leadStatusService.CacheData.objSearch;
      this.permission = this.leadStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leadStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leadStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leadStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leadStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leadStatusService.CacheData.Data, this.leadStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/crm/config/lead-statuses/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/crm/config/lead-statuses/view/' + obj.Id]);
    } 
