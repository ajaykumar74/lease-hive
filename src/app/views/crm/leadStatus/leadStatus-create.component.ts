...
@Component({
  selector: 'app-leadStatus-create',
  standalone: false,
  templateUrl: './leadStatus-create.component.html' ,
   providers: [ MessageService]
})
export class LeadStatusCreateComponent implements OnInit {
...
EffectiveTo:  obj.EffectiveTo || new Date(),
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/leadStatuss/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
EffectiveTo: formValues.EffectiveTo || null,
	  ...
	  this.spinner.show(); 
    this.leadStatusService.create(createdObj).subscribe({
      next: data => {	   
