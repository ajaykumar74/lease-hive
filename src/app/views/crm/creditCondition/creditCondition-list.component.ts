...
@Component({
  ...
  standalone: false,
  templateUrl: './creditCondition-list.component.html'
})
export class CreditConditionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditConditionService.CacheData.IsLoaded) {
      this.currentPage = this.creditConditionService.CacheData.CurrentPage;
      this.objSearch = this.creditConditionService.CacheData.objSearch;
      this.permission = this.creditConditionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditConditionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditConditionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditConditionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditConditionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditConditionService.CacheData.Data, this.creditConditionService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'ConditionType', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
