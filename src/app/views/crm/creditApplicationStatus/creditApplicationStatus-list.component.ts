...
@Component({
  ...
  standalone: false,
  templateUrl: './creditApplicationStatus-list.component.html'
})
export class CreditApplicationStatusListComponent implements OnInit {
  ...
  lstMain: ICreditApplicationStatus[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.creditApplicationStatusService.CacheData.IsLoaded) {
      this.currentPage = this.creditApplicationStatusService.CacheData.CurrentPage;
      this.objSearch = this.creditApplicationStatusService.CacheData.objSearch;
      this.permission = this.creditApplicationStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditApplicationStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditApplicationStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditApplicationStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditApplicationStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditApplicationStatusService.CacheData.Data, this.creditApplicationStatusService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'StatusName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'StatusCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/config/credit-statuses/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/config/credit-statuses/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/business/origination/config/credit-statuses/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
