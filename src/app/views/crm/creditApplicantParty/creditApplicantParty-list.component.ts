...
@Component({
  ...
  standalone: false,
  templateUrl: './creditApplicantParty-list.component.html'
})
export class CreditApplicantPartyListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditApplicantPartyService.CacheData.IsLoaded) {
      this.currentPage = this.creditApplicantPartyService.CacheData.CurrentPage;
      this.objSearch = this.creditApplicantPartyService.CacheData.objSearch;
      this.permission = this.creditApplicantPartyService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditApplicantPartyService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditApplicantPartyService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditApplicantPartyService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditApplicantPartyService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditApplicantPartyService.CacheData.Data, this.creditApplicantPartyService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/credit/ApplicantPartys/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/credit/ApplicantPartys/view/' + obj.Id]);
    } 
