...
@Component({
    templateUrl: './quoteTax-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class QuoteTaxViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    quoteTax: IQuoteTax = {} as IQuoteTax;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/quoteTax/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
