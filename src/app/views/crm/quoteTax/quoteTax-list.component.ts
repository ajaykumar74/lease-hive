...
@Component({
  ...
  standalone: false,
  templateUrl: './quoteTax-list.component.html'
})
export class QuoteTaxListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.quoteTaxService.CacheData.IsLoaded) {
      this.currentPage = this.quoteTaxService.CacheData.CurrentPage;
      this.objSearch = this.quoteTaxService.CacheData.objSearch;
      this.permission = this.quoteTaxService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.quoteTaxService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.quoteTaxService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.quoteTaxService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.quoteTaxService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.quoteTaxService.CacheData.Data, this.quoteTaxService.CacheData.TotalRecords);
    }
  }
