...
@Component({
  ...
  standalone: false,
  templateUrl: './originationDocumentLink-list.component.html'
})
export class OriginationDocumentLinkListComponent implements OnInit {
  ...
  lstMain: IOriginationDocumentLink[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.originationDocumentLinkService.CacheData.IsLoaded) {
      this.currentPage = this.originationDocumentLinkService.CacheData.CurrentPage;
      this.objSearch = this.originationDocumentLinkService.CacheData.objSearch;
      this.permission = this.originationDocumentLinkService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.originationDocumentLinkService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.originationDocumentLinkService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.originationDocumentLinkService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.originationDocumentLinkService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.originationDocumentLinkService.CacheData.Data, this.originationDocumentLinkService.CacheData.TotalRecords);
    }
  }
