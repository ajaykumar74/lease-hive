...
@Component({
  ...
  standalone: false,
  templateUrl: './quoteCharge-list.component.html'
})
export class QuoteChargeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.quoteChargeService.CacheData.IsLoaded) {
      this.currentPage = this.quoteChargeService.CacheData.CurrentPage;
      this.objSearch = this.quoteChargeService.CacheData.objSearch;
      this.permission = this.quoteChargeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.quoteChargeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.quoteChargeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.quoteChargeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.quoteChargeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.quoteChargeService.CacheData.Data, this.quoteChargeService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['origination/quotes/charges/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['origination/quotes/charges/view/' + obj.Id]);
    } 
