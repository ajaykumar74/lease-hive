...
@Component({
  ...
  standalone: false,
  templateUrl: './creditBureauResult-list.component.html'
})
export class CreditBureauResultListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditBureauResultService.CacheData.IsLoaded) {
      this.currentPage = this.creditBureauResultService.CacheData.CurrentPage;
      this.objSearch = this.creditBureauResultService.CacheData.objSearch;
      this.permission = this.creditBureauResultService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditBureauResultService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditBureauResultService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditBureauResultService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditBureauResultService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditBureauResultService.CacheData.Data, this.creditBureauResultService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['origination/credit/CreditBureauResult/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['origination/credit/CreditBureauResult/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['origination/credit/CreditBureauResult/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
