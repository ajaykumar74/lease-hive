...
@Component({
  ...
  standalone: false,
  templateUrl: './quoteDiscount-list.component.html'
})
export class QuoteDiscountListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.quoteDiscountService.CacheData.IsLoaded) {
      this.currentPage = this.quoteDiscountService.CacheData.CurrentPage;
      this.objSearch = this.quoteDiscountService.CacheData.objSearch;
      this.permission = this.quoteDiscountService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.quoteDiscountService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.quoteDiscountService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.quoteDiscountService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.quoteDiscountService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.quoteDiscountService.CacheData.Data, this.quoteDiscountService.CacheData.TotalRecords);
    }
  }
