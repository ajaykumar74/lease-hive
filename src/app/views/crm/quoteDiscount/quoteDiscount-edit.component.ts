...
@Component({
  selector: 'app-quoteDiscount-edit',
  standalone: false,
  templateUrl: './quoteDiscount-edit.component.html',
  providers: [ MessageService]
})
export class QuoteDiscountEditComponent implements OnInit {
...
this.approvedbyOptions.push({Text: 'AppUser1', Value: 'AppUser1' });
this.approvedbyOptions.push({Text: 'AppUser2', Value: 'AppUser2' });
...
ApprovedOn:  obj.ApprovedOn || new Date(),
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/quoteDiscount/create', { id: -1 }]);
    }
    else if (key == "Save") {
      this.Save();
    }
...
ApprovedOn:  formValues.ApprovedOn || null,
	...
	this.spinner.show();  	   
    this.quoteDiscountService.update(this.quoteDiscount.Id, updatedObj).subscribe({
      next: data => {
