...
@Component({
  ...
  standalone: false,
  templateUrl: './opportunityStage-list.component.html'
})
export class OpportunityStageListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.opportunityStageService.CacheData.IsLoaded) {
      this.currentPage = this.opportunityStageService.CacheData.CurrentPage;
      this.objSearch = this.opportunityStageService.CacheData.objSearch;
      this.permission = this.opportunityStageService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.opportunityStageService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.opportunityStageService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.opportunityStageService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.opportunityStageService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.opportunityStageService.CacheData.Data, this.opportunityStageService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'StageName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'StageCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/crm/config/opportunity-stages/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/crm/config/opportunity-stages/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/business/crm/config/opportunity-stages/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
