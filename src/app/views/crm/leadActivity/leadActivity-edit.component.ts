...
@Component({
  selector: 'app-leadActivity-edit',
  standalone: false,
  templateUrl: './leadActivity-edit.component.html',
  providers: [ MessageService]
})
export class LeadActivityEditComponent implements OnInit {
...
Description: new FormControl('', [Validators.maxLength(100), ]), 
   ...
   this.leadidOptions.push({Text: 'Lead1', Value: 'Lead1' });
this.leadidOptions.push({Text: 'Lead2', Value: 'Lead2' });
...
this.recordstatusOptions.push({Text: 'Disabled', Value: 'Disabled' });
  ...
  loadUI(): void {
    ...
    this.leadActivityService.getById(this.selectedId).subscribe({
      next: data => {	        
        this.leadActivity = data.data;
		this.permission = data.permission;
        this.objMaster = { ...this.leadActivity };
        this.populateUI(this.leadActivity);
      },
...
Description: obj.Description || '',
	 ...
	 this.Caption = "LeadActivity Details #" + obj.Id;
  } 

  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/business/crm/activities/create', { id: -1 }]);
    }
    else if (key == "Save") {
      this.Save();
    }
...
AssignedUserId:  formValues.AssignedUserId || null,
...
Description:  formValues.Description || null,
	...
	this.spinner.show();  	   
    this.leadActivityService.update(this.leadActivity.Id, updatedObj).subscribe({
      next: data => {
