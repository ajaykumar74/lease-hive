...
@Component({
  ...
  standalone: false,
  templateUrl: './creditApplication-list.component.html'
})
export class CreditApplicationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditApplicationService.CacheData.IsLoaded) {
      this.currentPage = this.creditApplicationService.CacheData.CurrentPage;
      this.objSearch = this.creditApplicationService.CacheData.objSearch;
      this.permission = this.creditApplicationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditApplicationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditApplicationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditApplicationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditApplicationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditApplicationService.CacheData.Data, this.creditApplicationService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/credit/applications/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/credit/applications/view/' + obj.Id]);
    } 
