...
@Component({
  ...
  standalone: false,
  templateUrl: './creditFinancialSnapshot-list.component.html'
})
export class CreditFinancialSnapshotListComponent implements OnInit {
  ...
  lstMain: ICreditFinancialSnapshot[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.creditFinancialSnapshotService.CacheData.IsLoaded) {
      this.currentPage = this.creditFinancialSnapshotService.CacheData.CurrentPage;
      this.objSearch = this.creditFinancialSnapshotService.CacheData.objSearch;
      this.permission = this.creditFinancialSnapshotService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditFinancialSnapshotService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditFinancialSnapshotService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditFinancialSnapshotService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditFinancialSnapshotService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditFinancialSnapshotService.CacheData.Data, this.creditFinancialSnapshotService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['origination/credit/snapshot/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['origination/credit/snapshot/view/' + obj.Id]);
    } 
