...
@Component({
  ...
  standalone: false,
  templateUrl: './creditExposure-list.component.html'
})
export class CreditExposureListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditExposureService.CacheData.IsLoaded) {
      this.currentPage = this.creditExposureService.CacheData.CurrentPage;
      this.objSearch = this.creditExposureService.CacheData.objSearch;
      this.permission = this.creditExposureService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditExposureService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditExposureService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditExposureService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditExposureService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditExposureService.CacheData.Data, this.creditExposureService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['origination/credit/exposure/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['origination/credit/exposure/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['origination/credit/exposure/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
