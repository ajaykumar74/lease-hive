...
@Component({
  ...
  standalone: false,
  templateUrl: './creditLimit-list.component.html'
})
export class CreditLimitListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditLimitService.CacheData.IsLoaded) {
      this.currentPage = this.creditLimitService.CacheData.CurrentPage;
      this.objSearch = this.creditLimitService.CacheData.objSearch;
      this.permission = this.creditLimitService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditLimitService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditLimitService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditLimitService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditLimitService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditLimitService.CacheData.Data, this.creditLimitService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/credit/limits/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/credit/limits/view/' + obj.Id]);
    } 
