...
@Component({
  ...
  standalone: false,
  templateUrl: './originationHandoff-list.component.html'
})
export class OriginationHandoffListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.originationHandoffService.CacheData.IsLoaded) {
      this.currentPage = this.originationHandoffService.CacheData.CurrentPage;
      this.objSearch = this.originationHandoffService.CacheData.objSearch;
      this.permission = this.originationHandoffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.originationHandoffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.originationHandoffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.originationHandoffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.originationHandoffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.originationHandoffService.CacheData.Data, this.originationHandoffService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/handoffs/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/handoffs/view/' + obj.Id]);
    } 
