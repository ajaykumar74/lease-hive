...
@Component({
  ...
  standalone: false,
  templateUrl: './approvalRequest-list.component.html'
})
export class ApprovalRequestListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.approvalRequestService.CacheData.IsLoaded) {
      this.currentPage = this.approvalRequestService.CacheData.CurrentPage;
      this.objSearch = this.approvalRequestService.CacheData.objSearch;
      this.permission = this.approvalRequestService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.approvalRequestService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.approvalRequestService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.approvalRequestService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.approvalRequestService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.approvalRequestService.CacheData.Data, this.approvalRequestService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
     ...
     // { DBName: 'Name', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'ReferenceType', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/credit/approvals/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/credit/approvals/view/' + obj.Id]);
    } 
