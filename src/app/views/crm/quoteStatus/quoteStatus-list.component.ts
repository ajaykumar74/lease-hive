...
@Component({
  ...
  standalone: false,
  templateUrl: './quoteStatus-list.component.html'
})
export class QuoteStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.quoteStatusService.CacheData.IsLoaded) {
      this.currentPage = this.quoteStatusService.CacheData.CurrentPage;
      this.objSearch = this.quoteStatusService.CacheData.objSearch;
      this.permission = this.quoteStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.quoteStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.quoteStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.quoteStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.quoteStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.quoteStatusService.CacheData.Data, this.quoteStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/origination/config/quote-statuses/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/origination/config/quote-statuses/view/' + obj.Id]);
    } 
