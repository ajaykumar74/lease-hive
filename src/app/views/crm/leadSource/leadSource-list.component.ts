...
@Component({
  ...
  standalone: false,
  templateUrl: './leadSource-list.component.html'
})
export class LeadSourceListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leadSourceService.CacheData.IsLoaded) {
      this.currentPage = this.leadSourceService.CacheData.CurrentPage;
      this.objSearch = this.leadSourceService.CacheData.objSearch;
      this.permission = this.leadSourceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leadSourceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leadSourceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leadSourceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leadSourceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leadSourceService.CacheData.Data, this.leadSourceService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'SourceName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'SourceCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/crm/config/lead-sources/edit', obj.Id]);
    }
    else {
        this.router.navigate(['/business/crm/config/lead-sources/view', obj.Id]);
    } 
