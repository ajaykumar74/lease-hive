...
@Component({
  ...
  standalone: false,
  templateUrl: './creditDecision-list.component.html'
})
export class CreditDecisionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditDecisionService.CacheData.IsLoaded) {
      this.currentPage = this.creditDecisionService.CacheData.CurrentPage;
      this.objSearch = this.creditDecisionService.CacheData.objSearch;
      this.permission = this.creditDecisionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditDecisionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditDecisionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditDecisionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditDecisionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditDecisionService.CacheData.Data, this.creditDecisionService.CacheData.TotalRecords);
    }
  }
