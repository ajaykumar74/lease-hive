...
export class UserRoleListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.userRoleService.CacheData.IsLoaded) {
      this.currentPage = this.userRoleService.CacheData.CurrentPage;
      this.objSearch = this.userRoleService.CacheData.objSearch;
      this.permission = this.userRoleService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.userRoleService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.userRoleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.userRoleService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.userRoleService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.userRoleService.CacheData.Data, this.userRoleService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/userRoles/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/userRoles/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/userRoles/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
