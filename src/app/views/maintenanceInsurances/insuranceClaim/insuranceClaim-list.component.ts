...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceClaim-list.component.html'
})
export class InsuranceClaimListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insuranceClaimService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceClaimService.CacheData.CurrentPage;
      this.objSearch = this.insuranceClaimService.CacheData.objSearch;
      this.permission = this.insuranceClaimService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceClaimService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceClaimService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceClaimService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceClaimService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceClaimService.CacheData.Data, this.insuranceClaimService.CacheData.TotalRecords);
    }
  }
