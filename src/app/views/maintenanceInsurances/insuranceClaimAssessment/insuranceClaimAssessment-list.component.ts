...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceClaimAssessment-list.component.html'
})
export class InsuranceClaimAssessmentListComponent implements OnInit {
  ...
  lstMain: IInsuranceClaimAssessment[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.insuranceClaimAssessmentService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceClaimAssessmentService.CacheData.CurrentPage;
      this.objSearch = this.insuranceClaimAssessmentService.CacheData.objSearch;
      this.permission = this.insuranceClaimAssessmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceClaimAssessmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceClaimAssessmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceClaimAssessmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceClaimAssessmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceClaimAssessmentService.CacheData.Data, this.insuranceClaimAssessmentService.CacheData.TotalRecords);
    }
  }
