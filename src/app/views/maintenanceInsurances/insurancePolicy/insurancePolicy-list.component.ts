...
export class InsurancePolicyListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insurancePolicyService.CacheData.IsLoaded) {
      this.currentPage = this.insurancePolicyService.CacheData.CurrentPage;
      this.objSearch = this.insurancePolicyService.CacheData.objSearch;
      this.permission = this.insurancePolicyService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insurancePolicyService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insurancePolicyService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insurancePolicyService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insurancePolicyService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insurancePolicyService.CacheData.Data, this.insurancePolicyService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/maintenance-insurance/insurance/policies/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/maintenance-insurance/insurance/policies/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenance-insurance/insurance/policies/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
