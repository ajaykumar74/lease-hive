...
export class MaintenanceScheduleListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.maintenanceScheduleService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceScheduleService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceScheduleService.CacheData.objSearch;
      this.permission = this.maintenanceScheduleService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceScheduleService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceScheduleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceScheduleService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceScheduleService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceScheduleService.CacheData.Data, this.maintenanceScheduleService.CacheData.TotalRecords);
    }
  }
