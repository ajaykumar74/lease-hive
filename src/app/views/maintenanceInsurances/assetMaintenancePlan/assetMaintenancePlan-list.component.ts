...
@Component({
  ...
  standalone: false,
  templateUrl: './assetMaintenancePlan-list.component.html'
})
export class AssetMaintenancePlanListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetMaintenancePlanService.CacheData.IsLoaded) {
      this.currentPage = this.assetMaintenancePlanService.CacheData.CurrentPage;
      this.objSearch = this.assetMaintenancePlanService.CacheData.objSearch;
      this.permission = this.assetMaintenancePlanService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetMaintenancePlanService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetMaintenancePlanService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetMaintenancePlanService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetMaintenancePlanService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetMaintenancePlanService.CacheData.Data, this.assetMaintenancePlanService.CacheData.TotalRecords);
    }
  }
