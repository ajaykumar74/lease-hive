...
@Component({
    templateUrl: './maintenanceCompletion-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class MaintenanceCompletionViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    maintenanceCompletion: IMaintenanceCompletion = {} as IMaintenanceCompletion;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/maintenanceCompletion/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
