...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceRenewal-list.component.html'
})
export class InsuranceRenewalListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insuranceRenewalService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceRenewalService.CacheData.CurrentPage;
      this.objSearch = this.insuranceRenewalService.CacheData.objSearch;
      this.permission = this.insuranceRenewalService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceRenewalService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceRenewalService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceRenewalService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceRenewalService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceRenewalService.CacheData.Data, this.insuranceRenewalService.CacheData.TotalRecords);
    }
  }
