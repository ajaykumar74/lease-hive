...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceInsuranceHandoff-list.component.html'
})
export class MaintenanceInsuranceHandoffListComponent implements OnInit {
  ...
  lstMain: IMaintenanceInsuranceHandoff[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.maintenanceInsuranceHandoffService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceInsuranceHandoffService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceInsuranceHandoffService.CacheData.objSearch;
      this.permission = this.maintenanceInsuranceHandoffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceInsuranceHandoffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceInsuranceHandoffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceInsuranceHandoffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceInsuranceHandoffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceInsuranceHandoffService.CacheData.Data, this.maintenanceInsuranceHandoffService.CacheData.TotalRecords);
    }
  }
