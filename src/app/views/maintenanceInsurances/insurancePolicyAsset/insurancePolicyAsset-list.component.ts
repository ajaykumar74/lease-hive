...
@Component({
  ...
  standalone: false,
  templateUrl: './insurancePolicyAsset-list.component.html'
})
export class InsurancePolicyAssetListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insurancePolicyAssetService.CacheData.IsLoaded) {
      this.currentPage = this.insurancePolicyAssetService.CacheData.CurrentPage;
      this.objSearch = this.insurancePolicyAssetService.CacheData.objSearch;
      this.permission = this.insurancePolicyAssetService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insurancePolicyAssetService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insurancePolicyAssetService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insurancePolicyAssetService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insurancePolicyAssetService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insurancePolicyAssetService.CacheData.Data, this.insurancePolicyAssetService.CacheData.TotalRecords);
    }
  }
