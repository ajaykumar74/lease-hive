...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceClaimStatus-list.component.html'
})
export class InsuranceClaimStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insuranceClaimStatusService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceClaimStatusService.CacheData.CurrentPage;
      this.objSearch = this.insuranceClaimStatusService.CacheData.objSearch;
      this.permission = this.insuranceClaimStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceClaimStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceClaimStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceClaimStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceClaimStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceClaimStatusService.CacheData.Data, this.insuranceClaimStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/insuranceClaimStatuss/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/insuranceClaimStatuss/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/insuranceClaimStatuss/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
