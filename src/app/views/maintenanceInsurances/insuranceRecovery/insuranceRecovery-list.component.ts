...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceRecovery-list.component.html'
})
export class InsuranceRecoveryListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insuranceRecoveryService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceRecoveryService.CacheData.CurrentPage;
      this.objSearch = this.insuranceRecoveryService.CacheData.objSearch;
      this.permission = this.insuranceRecoveryService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceRecoveryService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceRecoveryService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceRecoveryService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceRecoveryService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceRecoveryService.CacheData.Data, this.insuranceRecoveryService.CacheData.TotalRecords);
    }
  }
