...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceWorkOrderPart-list.component.html'
})
export class MaintenanceWorkOrderPartListComponent implements OnInit {
  ...
  lstMain: IMaintenanceWorkOrderPart[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.maintenanceWorkOrderPartService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceWorkOrderPartService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceWorkOrderPartService.CacheData.objSearch;
      this.permission = this.maintenanceWorkOrderPartService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceWorkOrderPartService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceWorkOrderPartService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceWorkOrderPartService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceWorkOrderPartService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceWorkOrderPartService.CacheData.Data, this.maintenanceWorkOrderPartService.CacheData.TotalRecords);
    }
  }
