...
@Component({
  ...
  standalone: false,
  templateUrl: './insurancePolicyEndorsement-list.component.html'
})
export class InsurancePolicyEndorsementListComponent implements OnInit {
  ...
  lstMain: IInsurancePolicyEndorsement[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.insurancePolicyEndorsementService.CacheData.IsLoaded) {
      this.currentPage = this.insurancePolicyEndorsementService.CacheData.CurrentPage;
      this.objSearch = this.insurancePolicyEndorsementService.CacheData.objSearch;
      this.permission = this.insurancePolicyEndorsementService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insurancePolicyEndorsementService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insurancePolicyEndorsementService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insurancePolicyEndorsementService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insurancePolicyEndorsementService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insurancePolicyEndorsementService.CacheData.Data, this.insurancePolicyEndorsementService.CacheData.TotalRecords);
    }
  }
