...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceIncident-list.component.html'
})
export class InsuranceIncidentListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.insuranceIncidentService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceIncidentService.CacheData.CurrentPage;
      this.objSearch = this.insuranceIncidentService.CacheData.objSearch;
      this.permission = this.insuranceIncidentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceIncidentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceIncidentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceIncidentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceIncidentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceIncidentService.CacheData.Data, this.insuranceIncidentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/insuranceIncidents/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/insuranceIncidents/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/insuranceIncidents/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
