...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenancePlan-list.component.html'
})
export class MaintenancePlanListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.maintenancePlanService.CacheData.IsLoaded) {
      this.currentPage = this.maintenancePlanService.CacheData.CurrentPage;
      this.objSearch = this.maintenancePlanService.CacheData.objSearch;
      this.permission = this.maintenancePlanService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenancePlanService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenancePlanService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenancePlanService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenancePlanService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenancePlanService.CacheData.Data, this.maintenancePlanService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/maintenance-insurance/maintenance/plans/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/maintenance-insurance/maintenance/plans/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenance-insurance/maintenance/plans/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
