...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceInsuranceDocumentLink-list.component.html'
})
export class MaintenanceInsuranceDocumentLinkListComponent implements OnInit {
  ...
  lstMain: IMaintenanceInsuranceDocumentLink[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.maintenanceInsuranceDocumentLinkService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceInsuranceDocumentLinkService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceInsuranceDocumentLinkService.CacheData.objSearch;
      this.permission = this.maintenanceInsuranceDocumentLinkService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceInsuranceDocumentLinkService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceInsuranceDocumentLinkService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceInsuranceDocumentLinkService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceInsuranceDocumentLinkService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceInsuranceDocumentLinkService.CacheData.Data, this.maintenanceInsuranceDocumentLinkService.CacheData.TotalRecords);
    }
  }
