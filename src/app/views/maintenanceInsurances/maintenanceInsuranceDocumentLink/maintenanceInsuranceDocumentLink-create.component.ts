...
@Component({
  selector: 'app-maintenanceInsuranceDocumentLink-create',
  standalone: false,
  templateUrl: './maintenanceInsuranceDocumentLink-create.component.html' ,
   providers: [ MessageService]
})
export class MaintenanceInsuranceDocumentLinkCreateComponent implements OnInit {
...
this.recordstatusOptions.push({Text: 'Archived', Value: 'Archived' });
 ...
 loadUI(): void {
    ...
    this.maintenanceInsuranceDocumentLinkService.getById(this.selectedId).subscribe({
      next: data => {
        this.maintenanceInsuranceDocumentLink = data;
        this.objMaster = { ...this.maintenanceInsuranceDocumentLink };
...
RecordStatus: obj.RecordStatus || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenanceInsuranceDocumentLinks/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
RecordStatus: formValues.RecordStatus || null,
	  ...
	  this.spinner.show(); 
    this.maintenanceInsuranceDocumentLinkService.create(createdObj).subscribe({
      next: data => {	   
