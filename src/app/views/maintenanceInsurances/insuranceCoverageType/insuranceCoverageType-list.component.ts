...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceCoverageType-list.component.html'
})
export class InsuranceCoverageTypeListComponent implements OnInit {
  ...
  lstMain: IInsuranceCoverageType[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.insuranceCoverageTypeService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceCoverageTypeService.CacheData.CurrentPage;
      this.objSearch = this.insuranceCoverageTypeService.CacheData.objSearch;
      this.permission = this.insuranceCoverageTypeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceCoverageTypeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceCoverageTypeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceCoverageTypeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceCoverageTypeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceCoverageTypeService.CacheData.Data, this.insuranceCoverageTypeService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/insuranceCoverageTypes/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/insuranceCoverageTypes/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/insuranceCoverageTypes/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
