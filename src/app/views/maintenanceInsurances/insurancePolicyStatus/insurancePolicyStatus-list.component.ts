...
@Component({
  ...
  standalone: false,
  templateUrl: './insurancePolicyStatus-list.component.html'
})
export class InsurancePolicyStatusListComponent implements OnInit {
  ...
  lstMain: IInsurancePolicyStatus[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.insurancePolicyStatusService.CacheData.IsLoaded) {
      this.currentPage = this.insurancePolicyStatusService.CacheData.CurrentPage;
      this.objSearch = this.insurancePolicyStatusService.CacheData.objSearch;
      this.permission = this.insurancePolicyStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insurancePolicyStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insurancePolicyStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insurancePolicyStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insurancePolicyStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insurancePolicyStatusService.CacheData.Data, this.insurancePolicyStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/insurancePolicyStatuss/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/insurancePolicyStatuss/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/insurancePolicyStatuss/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
