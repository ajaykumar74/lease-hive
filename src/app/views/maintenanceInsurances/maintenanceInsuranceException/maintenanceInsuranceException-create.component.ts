...
@Component({
  selector: 'app-maintenanceInsuranceException-create',
  standalone: false,
  templateUrl: './maintenanceInsuranceException-create.component.html' ,
...
this.recordstatusOptions = this.loggedInUserService.getPicklistOptions('RecordStatus');
 ...
 loadUI(): void {
    ...
    this.maintenanceInsuranceExceptionService.getById(this.selectedId).subscribe({
      next: data => {
        this.maintenanceInsuranceException = data;
        this.objMaster = { ...this.maintenanceInsuranceException };
...
RecordStatus: obj.RecordStatus || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenanceInsuranceExceptions/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
RecordStatus: formValues.RecordStatus || null,
	  ...
	  this.spinner.show(); 
    this.maintenanceInsuranceExceptionService.create(createdObj).subscribe({
      next: data => {	   
