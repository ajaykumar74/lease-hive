...
@Component({
  selector: 'app-maintenanceWorkOrderService-create',
  standalone: false,
  templateUrl: './maintenanceWorkOrderService-create.component.html' ,
   providers: [ MessageService]
})
export class MaintenanceWorkOrderServiceCreateComponent implements OnInit {
...
this.recordstatusOptions = this.loggedInUserService.getPicklistOptions('RecordStatus');
 ...
 loadUI(): void {
    ...
    this.maintenanceWorkOrderServiceService.getById(this.selectedId).subscribe({
      next: data => {
        this.maintenanceWorkOrderService = data;
        this.objMaster = { ...this.maintenanceWorkOrderService };
...
RecordStatus: obj.RecordStatus || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenanceWorkOrderServices/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
RecordStatus: formValues.RecordStatus || null,
	  ...
	  this.spinner.show(); 
    this.maintenanceWorkOrderServiceService.create(createdObj).subscribe({
      next: data => {	   
