...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceWorkOrderService-list.component.html'
})
export class MaintenanceWorkOrderServiceListComponent implements OnInit {
  ...
  lstMain: IMaintenanceWorkOrderService[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.maintenanceWorkOrderServiceService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceWorkOrderServiceService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceWorkOrderServiceService.CacheData.objSearch;
      this.permission = this.maintenanceWorkOrderServiceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceWorkOrderServiceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceWorkOrderServiceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceWorkOrderServiceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceWorkOrderServiceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceWorkOrderServiceService.CacheData.Data, this.maintenanceWorkOrderServiceService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/maintenance-insurance/maintenance/work-orders/services/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/maintenance-insurance/maintenance/work-orders/services/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenance-insurance/maintenance/work-orders/services/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
