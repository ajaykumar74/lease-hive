...
@Component({
  ...
  standalone: false,
  templateUrl: './serviceAgreement-list.component.html'
})
export class ServiceAgreementListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.serviceAgreementService.CacheData.IsLoaded) {
      this.currentPage = this.serviceAgreementService.CacheData.CurrentPage;
      this.objSearch = this.serviceAgreementService.CacheData.objSearch;
      this.permission = this.serviceAgreementService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.serviceAgreementService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.serviceAgreementService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.serviceAgreementService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.serviceAgreementService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.serviceAgreementService.CacheData.Data, this.serviceAgreementService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/maintenance-insurance/maintenance/service-agreements/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/maintenance-insurance/maintenance/service-agreements/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenance-insurance/maintenance/service-agreements/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
