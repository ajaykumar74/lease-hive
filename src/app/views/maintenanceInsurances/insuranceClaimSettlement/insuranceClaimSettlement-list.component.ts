...
@Component({
  ...
  standalone: false,
  templateUrl: './insuranceClaimSettlement-list.component.html'
})
export class InsuranceClaimSettlementListComponent implements OnInit {
  ...
  lstMain: IInsuranceClaimSettlement[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.insuranceClaimSettlementService.CacheData.IsLoaded) {
      this.currentPage = this.insuranceClaimSettlementService.CacheData.CurrentPage;
      this.objSearch = this.insuranceClaimSettlementService.CacheData.objSearch;
      this.permission = this.insuranceClaimSettlementService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.insuranceClaimSettlementService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.insuranceClaimSettlementService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.insuranceClaimSettlementService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.insuranceClaimSettlementService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.insuranceClaimSettlementService.CacheData.Data, this.insuranceClaimSettlementService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/maintenance-insurance/insurance/settlements/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/maintenance-insurance/insurance/settlements/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenance-insurance/insurance/settlements/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
