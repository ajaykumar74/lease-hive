...
@Component({
  ...
  standalone: false,
  templateUrl: './assetDowntime-list.component.html'
})
export class AssetDowntimeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetDowntimeService.CacheData.IsLoaded) {
      this.currentPage = this.assetDowntimeService.CacheData.CurrentPage;
      this.objSearch = this.assetDowntimeService.CacheData.objSearch;
      this.permission = this.assetDowntimeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetDowntimeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetDowntimeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetDowntimeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetDowntimeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetDowntimeService.CacheData.Data, this.assetDowntimeService.CacheData.TotalRecords);
    }
  }
