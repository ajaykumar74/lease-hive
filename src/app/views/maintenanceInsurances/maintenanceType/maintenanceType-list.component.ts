...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceType-list.component.html'
})
export class MaintenanceTypeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.maintenanceTypeService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceTypeService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceTypeService.CacheData.objSearch;
      this.permission = this.maintenanceTypeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceTypeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceTypeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceTypeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceTypeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceTypeService.CacheData.Data, this.maintenanceTypeService.CacheData.TotalRecords);
    }
  }
