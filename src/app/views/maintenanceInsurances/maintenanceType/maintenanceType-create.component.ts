...
@Component({
  selector: 'app-maintenanceType-create',
  standalone: false,
  templateUrl: './maintenanceType-create.component.html' ,
   providers: [ MessageService]
})
export class MaintenanceTypeCreateComponent implements OnInit {
...
RecordStatus: obj.RecordStatus || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/maintenanceTypes/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
RecordStatus: formValues.RecordStatus || null,
	  ...
	  this.spinner.show(); 
    this.maintenanceTypeService.create(createdObj).subscribe({
      next: data => {	   
