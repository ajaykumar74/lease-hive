...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceRequest-list.component.html'
})
export class MaintenanceRequestListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.maintenanceRequestService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceRequestService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceRequestService.CacheData.objSearch;
      this.permission = this.maintenanceRequestService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceRequestService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceRequestService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceRequestService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceRequestService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceRequestService.CacheData.Data, this.maintenanceRequestService.CacheData.TotalRecords);
    }
  }
