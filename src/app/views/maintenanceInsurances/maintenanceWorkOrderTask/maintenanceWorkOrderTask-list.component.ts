...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceWorkOrderTask-list.component.html'
})
export class MaintenanceWorkOrderTaskListComponent implements OnInit {
  ...
  lstMain: IMaintenanceWorkOrderTask[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.maintenanceWorkOrderTaskService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceWorkOrderTaskService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceWorkOrderTaskService.CacheData.objSearch;
      this.permission = this.maintenanceWorkOrderTaskService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceWorkOrderTaskService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceWorkOrderTaskService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceWorkOrderTaskService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceWorkOrderTaskService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceWorkOrderTaskService.CacheData.Data, this.maintenanceWorkOrderTaskService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/maintenanceWorkOrderTasks/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/maintenanceWorkOrderTasks/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/maintenanceWorkOrderTasks/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
