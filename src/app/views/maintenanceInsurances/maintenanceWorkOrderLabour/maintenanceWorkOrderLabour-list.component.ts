...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceWorkOrderLabour-list.component.html'
})
export class MaintenanceWorkOrderLabourListComponent implements OnInit {
  ...
  lstMain: IMaintenanceWorkOrderLabour[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.maintenanceWorkOrderLabourService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceWorkOrderLabourService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceWorkOrderLabourService.CacheData.objSearch;
      this.permission = this.maintenanceWorkOrderLabourService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceWorkOrderLabourService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceWorkOrderLabourService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceWorkOrderLabourService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceWorkOrderLabourService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceWorkOrderLabourService.CacheData.Data, this.maintenanceWorkOrderLabourService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/maintenance-insurance/maintenance/work-orders/labour/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/maintenance-insurance/maintenance/work-orders/labour/view/' + obj.Id]);
    } 
