...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceException-list.component.html'
})
export class MaintenanceExceptionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.maintenanceExceptionService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceExceptionService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceExceptionService.CacheData.objSearch;
      this.permission = this.maintenanceExceptionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceExceptionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceExceptionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceExceptionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceExceptionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceExceptionService.CacheData.Data, this.maintenanceExceptionService.CacheData.TotalRecords);
    }
  }
