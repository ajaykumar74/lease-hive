...
@Component({
    templateUrl: './maintenanceException-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class MaintenanceExceptionViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    maintenanceException: IMaintenanceException = {} as IMaintenanceException;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/maintenanceException/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
