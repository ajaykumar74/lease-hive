...
@Component({
  ...
  standalone: false,
  templateUrl: './maintenanceWorkOrder-list.component.html'
})
export class MaintenanceWorkOrderListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.maintenanceWorkOrderService.CacheData.IsLoaded) {
      this.currentPage = this.maintenanceWorkOrderService.CacheData.CurrentPage;
      this.objSearch = this.maintenanceWorkOrderService.CacheData.objSearch;
      this.permission = this.maintenanceWorkOrderService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.maintenanceWorkOrderService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.maintenanceWorkOrderService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.maintenanceWorkOrderService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.maintenanceWorkOrderService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.maintenanceWorkOrderService.CacheData.Data, this.maintenanceWorkOrderService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/maintenanceWorkOrders/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/maintenanceWorkOrders/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/maintenanceWorkOrders/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
