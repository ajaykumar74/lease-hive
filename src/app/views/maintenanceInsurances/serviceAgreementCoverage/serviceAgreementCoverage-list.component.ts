...
@Component({
  ...
  standalone: false,
  templateUrl: './serviceAgreementCoverage-list.component.html'
})
export class ServiceAgreementCoverageListComponent implements OnInit {
  ...
  lstMain: IServiceAgreementCoverage[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.serviceAgreementCoverageService.CacheData.IsLoaded) {
      this.currentPage = this.serviceAgreementCoverageService.CacheData.CurrentPage;
      this.objSearch = this.serviceAgreementCoverageService.CacheData.objSearch;
      this.permission = this.serviceAgreementCoverageService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.serviceAgreementCoverageService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.serviceAgreementCoverageService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.serviceAgreementCoverageService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.serviceAgreementCoverageService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.serviceAgreementCoverageService.CacheData.Data, this.serviceAgreementCoverageService.CacheData.TotalRecords);
    }
  }
