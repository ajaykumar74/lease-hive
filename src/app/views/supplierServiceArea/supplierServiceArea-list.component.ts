...
@Component({
  ...
  standalone: false,
  templateUrl: './supplierServiceArea-list.component.html'
})
export class SupplierServiceAreaListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.supplierServiceAreaService.CacheData.IsLoaded) {
      this.currentPage = this.supplierServiceAreaService.CacheData.CurrentPage;
      this.objSearch = this.supplierServiceAreaService.CacheData.objSearch;
      this.permission = this.supplierServiceAreaService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.supplierServiceAreaService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.supplierServiceAreaService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.supplierServiceAreaService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.supplierServiceAreaService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.supplierServiceAreaService.CacheData.Data, this.supplierServiceAreaService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'City', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'StateProvinceCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/supplierServiceAreas/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/supplierServiceAreas/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/supplierServiceAreas/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
