...
export class RolePermissionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.rolePermissionService.CacheData.IsLoaded) {
      this.currentPage = this.rolePermissionService.CacheData.CurrentPage;
      this.objSearch = this.rolePermissionService.CacheData.objSearch;
      this.permission = this.rolePermissionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.rolePermissionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.rolePermissionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.rolePermissionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.rolePermissionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.rolePermissionService.CacheData.Data, this.rolePermissionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/rolePermissions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/rolePermissions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/rolePermissions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
