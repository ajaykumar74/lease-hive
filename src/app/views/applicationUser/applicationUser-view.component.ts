...
@Component({
    templateUrl: './applicationUser-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class ApplicationUserViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    applicationUser: IApplicationUser = {} as IApplicationUser;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/applicationUser/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
