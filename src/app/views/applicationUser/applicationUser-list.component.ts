...
@Component({
  ...
  standalone: false,
  templateUrl: './applicationUser-list.component.html'
})
export class ApplicationUserListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.applicationUserService.CacheData.IsLoaded) {
      this.currentPage = this.applicationUserService.CacheData.CurrentPage;
      this.objSearch = this.applicationUserService.CacheData.objSearch;
      this.permission = this.applicationUserService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.applicationUserService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.applicationUserService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.applicationUserService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.applicationUserService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.applicationUserService.CacheData.Data, this.applicationUserService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'UserName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'Email', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/applicationUsers/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/applicationUsers/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/applicationUsers/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
