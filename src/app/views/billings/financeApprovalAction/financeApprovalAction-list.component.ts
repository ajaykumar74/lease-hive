...
@Component({
  ...
  standalone: false,
  templateUrl: './financeApprovalAction-list.component.html'
})
export class FinanceApprovalActionListComponent implements OnInit {
  ...
  lstMain: IFinanceApprovalAction[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.financeApprovalActionService.CacheData.IsLoaded) {
      this.currentPage = this.financeApprovalActionService.CacheData.CurrentPage;
      this.objSearch = this.financeApprovalActionService.CacheData.objSearch;
      this.permission = this.financeApprovalActionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.financeApprovalActionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.financeApprovalActionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.financeApprovalActionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.financeApprovalActionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.financeApprovalActionService.CacheData.Data, this.financeApprovalActionService.CacheData.TotalRecords);
    }
  }
