...
export class BankStatementListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.bankStatementService.CacheData.IsLoaded) {
      this.currentPage = this.bankStatementService.CacheData.CurrentPage;
      this.objSearch = this.bankStatementService.CacheData.objSearch;
      this.permission = this.bankStatementService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.bankStatementService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.bankStatementService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.bankStatementService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.bankStatementService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.bankStatementService.CacheData.Data, this.bankStatementService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/bankStatements/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/bankStatements/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/bankStatements/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
