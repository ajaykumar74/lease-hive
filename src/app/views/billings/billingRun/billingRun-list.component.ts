...
@Component({
  ...
  standalone: false,
  templateUrl: './billingRun-list.component.html'
})
export class BillingRunListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.billingRunService.CacheData.IsLoaded) {
      this.currentPage = this.billingRunService.CacheData.CurrentPage;
      this.objSearch = this.billingRunService.CacheData.objSearch;
      this.permission = this.billingRunService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.billingRunService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.billingRunService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.billingRunService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.billingRunService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.billingRunService.CacheData.Data, this.billingRunService.CacheData.TotalRecords);
    }
  }
