...
@Component({
  ...
  standalone: false,
  templateUrl: './bankStatementLine-list.component.html'
})
export class BankStatementLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.bankStatementLineService.CacheData.IsLoaded) {
      this.currentPage = this.bankStatementLineService.CacheData.CurrentPage;
      this.objSearch = this.bankStatementLineService.CacheData.objSearch;
      this.permission = this.bankStatementLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.bankStatementLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.bankStatementLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.bankStatementLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.bankStatementLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.bankStatementLineService.CacheData.Data, this.bankStatementLineService.CacheData.TotalRecords);
    }
  }
