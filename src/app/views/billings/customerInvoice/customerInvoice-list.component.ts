...
export class CustomerInvoiceListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerInvoiceService.CacheData.IsLoaded) {
      this.currentPage = this.customerInvoiceService.CacheData.CurrentPage;
      this.objSearch = this.customerInvoiceService.CacheData.objSearch;
      this.permission = this.customerInvoiceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerInvoiceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerInvoiceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.customerInvoiceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.customerInvoiceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerInvoiceService.CacheData.Data, this.customerInvoiceService.CacheData.TotalRecords);
    }
  }
