...
@Component({
  ...
  standalone: false,
  templateUrl: './paymentAllocation-list.component.html'
})
export class PaymentAllocationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.paymentAllocationService.CacheData.IsLoaded) {
      this.currentPage = this.paymentAllocationService.CacheData.CurrentPage;
      this.objSearch = this.paymentAllocationService.CacheData.objSearch;
      this.permission = this.paymentAllocationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.paymentAllocationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.paymentAllocationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.paymentAllocationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.paymentAllocationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.paymentAllocationService.CacheData.Data, this.paymentAllocationService.CacheData.TotalRecords);
    }
  }
