...
@Component({
  ...
  standalone: false,
  templateUrl: './customerStatementSnapshot-list.component.html'
})
export class CustomerStatementSnapshotListComponent implements OnInit {
  ...
  lstMain: ICustomerStatementSnapshot[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.customerStatementSnapshotService.CacheData.IsLoaded) {
      this.currentPage = this.customerStatementSnapshotService.CacheData.CurrentPage;
      this.objSearch = this.customerStatementSnapshotService.CacheData.objSearch;
      this.permission = this.customerStatementSnapshotService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerStatementSnapshotService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerStatementSnapshotService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.customerStatementSnapshotService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.customerStatementSnapshotService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerStatementSnapshotService.CacheData.Data, this.customerStatementSnapshotService.CacheData.TotalRecords);
    }
  }
