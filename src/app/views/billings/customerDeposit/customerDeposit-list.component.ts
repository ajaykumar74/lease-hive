...
@Component({
  ...
  standalone: false,
  templateUrl: './customerDeposit-list.component.html'
})
export class CustomerDepositListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerDepositService.CacheData.IsLoaded) {
      this.currentPage = this.customerDepositService.CacheData.CurrentPage;
      this.objSearch = this.customerDepositService.CacheData.objSearch;
      this.permission = this.customerDepositService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerDepositService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerDepositService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.customerDepositService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.customerDepositService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerDepositService.CacheData.Data, this.customerDepositService.CacheData.TotalRecords);
    }
  }
