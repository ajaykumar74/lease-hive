...
export class DebitNoteListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.debitNoteService.CacheData.IsLoaded) {
      this.currentPage = this.debitNoteService.CacheData.CurrentPage;
      this.objSearch = this.debitNoteService.CacheData.objSearch;
      this.permission = this.debitNoteService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.debitNoteService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.debitNoteService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.debitNoteService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.debitNoteService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.debitNoteService.CacheData.Data, this.debitNoteService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/debitNotes/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/debitNotes/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/debitNotes/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
