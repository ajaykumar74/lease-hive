...
@Component({
    templateUrl: './financeDocumentLink-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class FinanceDocumentLinkViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    financeDocumentLink: IFinanceDocumentLink = {} as IFinanceDocumentLink;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/financeDocumentLink/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
