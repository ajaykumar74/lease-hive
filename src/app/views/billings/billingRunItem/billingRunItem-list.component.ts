...
@Component({
  ...
  standalone: false,
  templateUrl: './billingRunItem-list.component.html'
})
export class BillingRunItemListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.billingRunItemService.CacheData.IsLoaded) {
      this.currentPage = this.billingRunItemService.CacheData.CurrentPage;
      this.objSearch = this.billingRunItemService.CacheData.objSearch;
      this.permission = this.billingRunItemService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.billingRunItemService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.billingRunItemService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.billingRunItemService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.billingRunItemService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.billingRunItemService.CacheData.Data, this.billingRunItemService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/billing-finance/billing/candidates/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/billing-finance/billing/candidates/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/billing-finance/billing/candidates/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
