...
@Component({
  ...
  standalone: false,
  templateUrl: './billingRunStatus-list.component.html'
})
export class BillingRunStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.billingRunStatusService.CacheData.IsLoaded) {
      this.currentPage = this.billingRunStatusService.CacheData.CurrentPage;
      this.objSearch = this.billingRunStatusService.CacheData.objSearch;
      this.permission = this.billingRunStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.billingRunStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.billingRunStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.billingRunStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.billingRunStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.billingRunStatusService.CacheData.Data, this.billingRunStatusService.CacheData.TotalRecords);
    }
  }
