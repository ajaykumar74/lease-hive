...
@Component({
  ...
  standalone: false,
  templateUrl: './depositTransaction-list.component.html'
})
export class DepositTransactionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.depositTransactionService.CacheData.IsLoaded) {
      this.currentPage = this.depositTransactionService.CacheData.CurrentPage;
      this.objSearch = this.depositTransactionService.CacheData.objSearch;
      this.permission = this.depositTransactionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.depositTransactionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.depositTransactionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.depositTransactionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.depositTransactionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.depositTransactionService.CacheData.Data, this.depositTransactionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/depositTransactions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/depositTransactions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/depositTransactions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
