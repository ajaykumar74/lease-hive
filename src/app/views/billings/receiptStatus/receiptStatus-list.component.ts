...
@Component({
  ...
  standalone: false,
  templateUrl: './receiptStatus-list.component.html'
})
export class ReceiptStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.receiptStatusService.CacheData.IsLoaded) {
      this.currentPage = this.receiptStatusService.CacheData.CurrentPage;
      this.objSearch = this.receiptStatusService.CacheData.objSearch;
      this.permission = this.receiptStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.receiptStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.receiptStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.receiptStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.receiptStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.receiptStatusService.CacheData.Data, this.receiptStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/billing-finance/configuration/receipt-statuses/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/billing-finance/configuration/receipt-statuses/view/' + obj.Id]);
    } 
