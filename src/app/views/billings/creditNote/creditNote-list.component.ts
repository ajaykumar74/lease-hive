...
export class CreditNoteListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditNoteService.CacheData.IsLoaded) {
      this.currentPage = this.creditNoteService.CacheData.CurrentPage;
      this.objSearch = this.creditNoteService.CacheData.objSearch;
      this.permission = this.creditNoteService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditNoteService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditNoteService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditNoteService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditNoteService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditNoteService.CacheData.Data, this.creditNoteService.CacheData.TotalRecords);
    }
  }
