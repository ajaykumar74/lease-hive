...
@Component({
  ...
  standalone: false,
  templateUrl: './invoiceStatus-list.component.html'
})
export class InvoiceStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.invoiceStatusService.CacheData.IsLoaded) {
      this.currentPage = this.invoiceStatusService.CacheData.CurrentPage;
      this.objSearch = this.invoiceStatusService.CacheData.objSearch;
      this.permission = this.invoiceStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.invoiceStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.invoiceStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.invoiceStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.invoiceStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.invoiceStatusService.CacheData.Data, this.invoiceStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/billing-finance/configuration/invoice-statuses/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/billing-finance/configuration/invoice-statuses/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/billing-finance/configuration/invoice-statuses/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
