...
@Component({
  ...
  standalone: false,
  templateUrl: './creditNoteLine-list.component.html'
})
export class CreditNoteLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.creditNoteLineService.CacheData.IsLoaded) {
      this.currentPage = this.creditNoteLineService.CacheData.CurrentPage;
      this.objSearch = this.creditNoteLineService.CacheData.objSearch;
      this.permission = this.creditNoteLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.creditNoteLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.creditNoteLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.creditNoteLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.creditNoteLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.creditNoteLineService.CacheData.Data, this.creditNoteLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/creditNoteLines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/creditNoteLines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/creditNoteLines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
