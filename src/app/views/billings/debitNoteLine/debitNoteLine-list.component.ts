...
@Component({
  ...
  standalone: false,
  templateUrl: './debitNoteLine-list.component.html'
})
export class DebitNoteLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.debitNoteLineService.CacheData.IsLoaded) {
      this.currentPage = this.debitNoteLineService.CacheData.CurrentPage;
      this.objSearch = this.debitNoteLineService.CacheData.objSearch;
      this.permission = this.debitNoteLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.debitNoteLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.debitNoteLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.debitNoteLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.debitNoteLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.debitNoteLineService.CacheData.Data, this.debitNoteLineService.CacheData.TotalRecords);
    }
  }
