...
@Component({
    templateUrl: './financeApprovalRequest-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class FinanceApprovalRequestViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    financeApprovalRequest: IFinanceApprovalRequest = {} as IFinanceApprovalRequest;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/financeApprovalRequest/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
