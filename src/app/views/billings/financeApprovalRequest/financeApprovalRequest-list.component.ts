...
@Component({
  ...
  standalone: false,
  templateUrl: './financeApprovalRequest-list.component.html'
})
export class FinanceApprovalRequestListComponent implements OnInit {
  ...
  lstMain: IFinanceApprovalRequest[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.financeApprovalRequestService.CacheData.IsLoaded) {
      this.currentPage = this.financeApprovalRequestService.CacheData.CurrentPage;
      this.objSearch = this.financeApprovalRequestService.CacheData.objSearch;
      this.permission = this.financeApprovalRequestService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.financeApprovalRequestService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.financeApprovalRequestService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.financeApprovalRequestService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.financeApprovalRequestService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.financeApprovalRequestService.CacheData.Data, this.financeApprovalRequestService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/financeApprovalRequests/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/financeApprovalRequests/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/financeApprovalRequests/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
