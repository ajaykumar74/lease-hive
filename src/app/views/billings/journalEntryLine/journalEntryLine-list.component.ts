...
@Component({
  ...
  standalone: false,
  templateUrl: './journalEntryLine-list.component.html'
})
export class JournalEntryLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.journalEntryLineService.CacheData.IsLoaded) {
      this.currentPage = this.journalEntryLineService.CacheData.CurrentPage;
      this.objSearch = this.journalEntryLineService.CacheData.objSearch;
      this.permission = this.journalEntryLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.journalEntryLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.journalEntryLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.journalEntryLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.journalEntryLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.journalEntryLineService.CacheData.Data, this.journalEntryLineService.CacheData.TotalRecords);
    }
  }
