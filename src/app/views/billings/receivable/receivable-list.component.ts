...
export class ReceivableListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.receivableService.CacheData.IsLoaded) {
      this.currentPage = this.receivableService.CacheData.CurrentPage;
      this.objSearch = this.receivableService.CacheData.objSearch;
      this.permission = this.receivableService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.receivableService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.receivableService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.receivableService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.receivableService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.receivableService.CacheData.Data, this.receivableService.CacheData.TotalRecords);
    }
  }
