...
@Component({
  ...
  standalone: false,
  templateUrl: './customerInvoiceLine-list.component.html'
})
export class CustomerInvoiceLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerInvoiceLineService.CacheData.IsLoaded) {
      this.currentPage = this.customerInvoiceLineService.CacheData.CurrentPage;
      this.objSearch = this.customerInvoiceLineService.CacheData.objSearch;
      this.permission = this.customerInvoiceLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerInvoiceLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerInvoiceLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.customerInvoiceLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.customerInvoiceLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerInvoiceLineService.CacheData.Data, this.customerInvoiceLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/billing-finance/invoices/lines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/billing-finance/invoices/lines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/billing-finance/invoices/lines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
