...
export class PaymentReceiptListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.paymentReceiptService.CacheData.IsLoaded) {
      this.currentPage = this.paymentReceiptService.CacheData.CurrentPage;
      this.objSearch = this.paymentReceiptService.CacheData.objSearch;
      this.permission = this.paymentReceiptService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.paymentReceiptService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.paymentReceiptService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.paymentReceiptService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.paymentReceiptService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.paymentReceiptService.CacheData.Data, this.paymentReceiptService.CacheData.TotalRecords);
    }
  }
