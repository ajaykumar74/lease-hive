...
@Component({
  ...
  standalone: false,
  templateUrl: './financeAccountMapping-list.component.html'
})
export class FinanceAccountMappingListComponent implements OnInit {
  ...
  lstMain: IFinanceAccountMapping[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.financeAccountMappingService.CacheData.IsLoaded) {
      this.currentPage = this.financeAccountMappingService.CacheData.CurrentPage;
      this.objSearch = this.financeAccountMappingService.CacheData.objSearch;
      this.permission = this.financeAccountMappingService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.financeAccountMappingService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.financeAccountMappingService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.financeAccountMappingService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.financeAccountMappingService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.financeAccountMappingService.CacheData.Data, this.financeAccountMappingService.CacheData.TotalRecords);
    }
  }
