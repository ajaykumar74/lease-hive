...
@Component({
  ...
  standalone: false,
  templateUrl: './customerInvoiceTax-list.component.html'
})
export class CustomerInvoiceTaxListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerInvoiceTaxService.CacheData.IsLoaded) {
      this.currentPage = this.customerInvoiceTaxService.CacheData.CurrentPage;
      this.objSearch = this.customerInvoiceTaxService.CacheData.objSearch;
      this.permission = this.customerInvoiceTaxService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerInvoiceTaxService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerInvoiceTaxService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.customerInvoiceTaxService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.customerInvoiceTaxService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerInvoiceTaxService.CacheData.Data, this.customerInvoiceTaxService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/customerInvoiceTaxs/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/customerInvoiceTaxs/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/customerInvoiceTaxs/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
