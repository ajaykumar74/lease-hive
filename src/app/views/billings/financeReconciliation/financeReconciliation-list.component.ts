...
@Component({
  ...
  standalone: false,
  templateUrl: './financeReconciliation-list.component.html'
})
export class FinanceReconciliationListComponent implements OnInit {
  ...
  lstMain: IFinanceReconciliation[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.financeReconciliationService.CacheData.IsLoaded) {
      this.currentPage = this.financeReconciliationService.CacheData.CurrentPage;
      this.objSearch = this.financeReconciliationService.CacheData.objSearch;
      this.permission = this.financeReconciliationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.financeReconciliationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.financeReconciliationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.financeReconciliationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.financeReconciliationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.financeReconciliationService.CacheData.Data, this.financeReconciliationService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/financeReconciliations/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/financeReconciliations/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/financeReconciliations/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
