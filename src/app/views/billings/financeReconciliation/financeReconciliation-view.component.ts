...
@Component({
    templateUrl: './financeReconciliation-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class FinanceReconciliationViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    financeReconciliation: IFinanceReconciliation = {} as IFinanceReconciliation;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/financeReconciliation/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
