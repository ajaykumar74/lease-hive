...
@Component({
  ...
  standalone: false,
  templateUrl: './financeException-list.component.html'
})
export class FinanceExceptionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.financeExceptionService.CacheData.IsLoaded) {
      this.currentPage = this.financeExceptionService.CacheData.CurrentPage;
      this.objSearch = this.financeExceptionService.CacheData.objSearch;
      this.permission = this.financeExceptionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.financeExceptionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.financeExceptionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.financeExceptionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.financeExceptionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.financeExceptionService.CacheData.Data, this.financeExceptionService.CacheData.TotalRecords);
    }
  }
