...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractStatus-list.component.html'
})
export class LeaseContractStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractStatusService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractStatusService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractStatusService.CacheData.objSearch;
      this.permission = this.leaseContractStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractStatusService.CacheData.Data, this.leaseContractStatusService.CacheData.TotalRecords);
    }
  }
