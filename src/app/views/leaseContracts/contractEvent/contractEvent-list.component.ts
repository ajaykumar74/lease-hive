...
@Component({
  ...
  standalone: false,
  templateUrl: './contractEvent-list.component.html'
})
export class ContractEventListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractEventService.CacheData.IsLoaded) {
      this.currentPage = this.contractEventService.CacheData.CurrentPage;
      this.objSearch = this.contractEventService.CacheData.objSearch;
      this.permission = this.contractEventService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractEventService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractEventService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractEventService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractEventService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractEventService.CacheData.Data, this.contractEventService.CacheData.TotalRecords);
    }
  }
