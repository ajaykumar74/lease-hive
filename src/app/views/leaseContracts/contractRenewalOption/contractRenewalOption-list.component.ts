...
@Component({
  ...
  standalone: false,
  templateUrl: './contractRenewalOption-list.component.html'
})
export class ContractRenewalOptionListComponent implements OnInit {
  ...
  lstMain: IContractRenewalOption[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractRenewalOptionService.CacheData.IsLoaded) {
      this.currentPage = this.contractRenewalOptionService.CacheData.CurrentPage;
      this.objSearch = this.contractRenewalOptionService.CacheData.objSearch;
      this.permission = this.contractRenewalOptionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractRenewalOptionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractRenewalOptionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractRenewalOptionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractRenewalOptionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractRenewalOptionService.CacheData.Data, this.contractRenewalOptionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractRenewalOptions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractRenewalOptions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractRenewalOptions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
