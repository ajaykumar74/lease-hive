...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractAsset-list.component.html'
})
export class LeaseContractAssetListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractAssetService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractAssetService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractAssetService.CacheData.objSearch;
      this.permission = this.leaseContractAssetService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractAssetService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractAssetService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractAssetService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractAssetService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractAssetService.CacheData.Data, this.leaseContractAssetService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/leaseContractAssets/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/leaseContractAssets/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/leaseContractAssets/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
