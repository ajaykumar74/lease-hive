...
@Component({
    templateUrl: './leaseContractAsset-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class LeaseContractAssetViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    leaseContractAsset: ILeaseContractAsset = {} as ILeaseContractAsset;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/leaseContractAsset/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
