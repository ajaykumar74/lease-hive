...
@Component({
  ...
  standalone: false,
  templateUrl: './contractExecution-list.component.html'
})
export class ContractExecutionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractExecutionService.CacheData.IsLoaded) {
      this.currentPage = this.contractExecutionService.CacheData.CurrentPage;
      this.objSearch = this.contractExecutionService.CacheData.objSearch;
      this.permission = this.contractExecutionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractExecutionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractExecutionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractExecutionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractExecutionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractExecutionService.CacheData.Data, this.contractExecutionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractExecutions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractExecutions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractExecutions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
