...
@Component({
  ...
  standalone: false,
  templateUrl: './leasePaymentScheduleLine-list.component.html'
})
export class LeasePaymentScheduleLineListComponent implements OnInit {
  ...
  lstMain: ILeasePaymentScheduleLine[]; 
  sortBy: string = 'Id';
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leasePaymentScheduleLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leasePaymentScheduleLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leasePaymentScheduleLineService.search(searchParam).subscribe({
        next: res => {
      ...
      this.SetListData(this.leasePaymentScheduleLineService.CacheData.Data, this.leasePaymentScheduleLineService.CacheData.TotalRecords);
    }
  }
