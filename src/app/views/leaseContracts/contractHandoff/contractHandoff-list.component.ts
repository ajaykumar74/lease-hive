...
@Component({
  ...
  standalone: false,
  templateUrl: './contractHandoff-list.component.html'
})
export class ContractHandoffListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractHandoffService.CacheData.IsLoaded) {
      this.currentPage = this.contractHandoffService.CacheData.CurrentPage;
      this.objSearch = this.contractHandoffService.CacheData.objSearch;
      this.permission = this.contractHandoffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractHandoffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractHandoffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractHandoffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractHandoffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractHandoffService.CacheData.Data, this.contractHandoffService.CacheData.TotalRecords);
    }
  }
