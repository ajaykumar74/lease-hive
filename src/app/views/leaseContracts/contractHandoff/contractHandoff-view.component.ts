...
@Component({
    templateUrl: './contractHandoff-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class ContractHandoffViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    contractHandoff: IContractHandoff = {} as IContractHandoff;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/contractHandoff/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
