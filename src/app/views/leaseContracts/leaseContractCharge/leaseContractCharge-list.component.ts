...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractCharge-list.component.html'
})
export class LeaseContractChargeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractChargeService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractChargeService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractChargeService.CacheData.objSearch;
      this.permission = this.leaseContractChargeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractChargeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractChargeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractChargeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractChargeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractChargeService.CacheData.Data, this.leaseContractChargeService.CacheData.TotalRecords);
    }
  }
