...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractDeposit-list.component.html'
})
export class LeaseContractDepositListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractDepositService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractDepositService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractDepositService.CacheData.objSearch;
      this.permission = this.leaseContractDepositService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractDepositService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractDepositService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractDepositService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractDepositService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractDepositService.CacheData.Data, this.leaseContractDepositService.CacheData.TotalRecords);
    }
  }
