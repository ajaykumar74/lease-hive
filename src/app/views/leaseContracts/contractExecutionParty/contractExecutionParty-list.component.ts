...
@Component({
  ...
  standalone: false,
  templateUrl: './contractExecutionParty-list.component.html'
})
export class ContractExecutionPartyListComponent implements OnInit {
  ...
  lstMain: IContractExecutionParty[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractExecutionPartyService.CacheData.IsLoaded) {
      this.currentPage = this.contractExecutionPartyService.CacheData.CurrentPage;
      this.objSearch = this.contractExecutionPartyService.CacheData.objSearch;
      this.permission = this.contractExecutionPartyService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractExecutionPartyService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractExecutionPartyService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractExecutionPartyService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractExecutionPartyService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractExecutionPartyService.CacheData.Data, this.contractExecutionPartyService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractExecutionPartys/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractExecutionPartys/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractExecutionPartys/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
