...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractDocumentLink-list.component.html'
})
export class LeaseContractDocumentLinkListComponent implements OnInit {
  ...
  lstMain: ILeaseContractDocumentLink[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.leaseContractDocumentLinkService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractDocumentLinkService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractDocumentLinkService.CacheData.objSearch;
      this.permission = this.leaseContractDocumentLinkService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractDocumentLinkService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractDocumentLinkService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractDocumentLinkService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractDocumentLinkService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractDocumentLinkService.CacheData.Data, this.leaseContractDocumentLinkService.CacheData.TotalRecords);
    }
  }
