...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseAssetAllocationEvent-list.component.html'
})
export class LeaseAssetAllocationEventListComponent implements OnInit {
  ...
  lstMain: ILeaseAssetAllocationEvent[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.leaseAssetAllocationEventService.CacheData.IsLoaded) {
      this.currentPage = this.leaseAssetAllocationEventService.CacheData.CurrentPage;
      this.objSearch = this.leaseAssetAllocationEventService.CacheData.objSearch;
      this.permission = this.leaseAssetAllocationEventService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseAssetAllocationEventService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseAssetAllocationEventService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseAssetAllocationEventService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseAssetAllocationEventService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseAssetAllocationEventService.CacheData.Data, this.leaseAssetAllocationEventService.CacheData.TotalRecords);
    }
  }
