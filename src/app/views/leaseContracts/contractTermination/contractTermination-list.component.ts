...
@Component({
  ...
  standalone: false,
  templateUrl: './contractTermination-list.component.html'
})
export class ContractTerminationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractTerminationService.CacheData.IsLoaded) {
      this.currentPage = this.contractTerminationService.CacheData.CurrentPage;
      this.objSearch = this.contractTerminationService.CacheData.objSearch;
      this.permission = this.contractTerminationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractTerminationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractTerminationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractTerminationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractTerminationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractTerminationService.CacheData.Data, this.contractTerminationService.CacheData.TotalRecords);
    }
  }
