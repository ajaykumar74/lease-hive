...
@Component({
  ...
  standalone: false,
  templateUrl: './contractAmendmentChange-list.component.html'
})
export class ContractAmendmentChangeListComponent implements OnInit {
  ...
  lstMain: IContractAmendmentChange[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractAmendmentChangeService.CacheData.IsLoaded) {
      this.currentPage = this.contractAmendmentChangeService.CacheData.CurrentPage;
      this.objSearch = this.contractAmendmentChangeService.CacheData.objSearch;
      this.permission = this.contractAmendmentChangeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractAmendmentChangeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractAmendmentChangeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractAmendmentChangeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractAmendmentChangeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractAmendmentChangeService.CacheData.Data, this.contractAmendmentChangeService.CacheData.TotalRecords);
    }
  }
