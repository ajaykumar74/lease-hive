...
@Component({
  ...
  standalone: false,
  templateUrl: './contractExternalReference-list.component.html'
})
export class ContractExternalReferenceListComponent implements OnInit {
  ...
  lstMain: IContractExternalReference[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractExternalReferenceService.CacheData.IsLoaded) {
      this.currentPage = this.contractExternalReferenceService.CacheData.CurrentPage;
      this.objSearch = this.contractExternalReferenceService.CacheData.objSearch;
      this.permission = this.contractExternalReferenceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractExternalReferenceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractExternalReferenceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractExternalReferenceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractExternalReferenceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractExternalReferenceService.CacheData.Data, this.contractExternalReferenceService.CacheData.TotalRecords);
    }
  }
