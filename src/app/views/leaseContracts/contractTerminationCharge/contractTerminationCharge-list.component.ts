...
@Component({
  ...
  standalone: false,
  templateUrl: './contractTerminationCharge-list.component.html'
})
export class ContractTerminationChargeListComponent implements OnInit {
  ...
  lstMain: IContractTerminationCharge[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractTerminationChargeService.CacheData.IsLoaded) {
      this.currentPage = this.contractTerminationChargeService.CacheData.CurrentPage;
      this.objSearch = this.contractTerminationChargeService.CacheData.objSearch;
      this.permission = this.contractTerminationChargeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractTerminationChargeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractTerminationChargeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractTerminationChargeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractTerminationChargeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractTerminationChargeService.CacheData.Data, this.contractTerminationChargeService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractTerminationCharges/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractTerminationCharges/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractTerminationCharges/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
