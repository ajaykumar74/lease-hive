...
@Component({
    templateUrl: './leaseContractParty-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class LeaseContractPartyViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    leaseContractParty: ILeaseContractParty = {} as ILeaseContractParty;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/leaseContractParty/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
