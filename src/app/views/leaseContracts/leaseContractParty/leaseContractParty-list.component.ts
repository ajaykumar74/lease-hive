...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractParty-list.component.html'
})
export class LeaseContractPartyListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractPartyService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractPartyService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractPartyService.CacheData.objSearch;
      this.permission = this.leaseContractPartyService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractPartyService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractPartyService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractPartyService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractPartyService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractPartyService.CacheData.Data, this.leaseContractPartyService.CacheData.TotalRecords);
    }
  }
