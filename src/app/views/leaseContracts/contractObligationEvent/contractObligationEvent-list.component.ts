...
@Component({
  ...
  standalone: false,
  templateUrl: './contractObligationEvent-list.component.html'
})
export class ContractObligationEventListComponent implements OnInit {
  ...
  lstMain: IContractObligationEvent[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractObligationEventService.CacheData.IsLoaded) {
      this.currentPage = this.contractObligationEventService.CacheData.CurrentPage;
      this.objSearch = this.contractObligationEventService.CacheData.objSearch;
      this.permission = this.contractObligationEventService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractObligationEventService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractObligationEventService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractObligationEventService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractObligationEventService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractObligationEventService.CacheData.Data, this.contractObligationEventService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractObligationEvents/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractObligationEvents/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractObligationEvents/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
