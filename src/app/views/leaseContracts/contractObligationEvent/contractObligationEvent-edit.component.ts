...
@Component({
  selector: 'app-contractObligationEvent-edit',
  standalone: false,
  templateUrl: './contractObligationEvent-edit.component.html',
...
PerformedBy: obj.PerformedBy || 0,
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/contracts/obligations/events/create', { id: -1 }]);
    }
    else if (key == "Save") {
      this.Save();
    }
...
PerformedBy:  formValues.PerformedBy || 0,
	...
	this.spinner.show();  	   
    this.contractObligationEventService.update(this.contractObligationEvent.Id, updatedObj).subscribe({
      next: data => {
