...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContractTerm-list.component.html'
})
export class LeaseContractTermListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractTermService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractTermService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractTermService.CacheData.objSearch;
      this.permission = this.leaseContractTermService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractTermService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractTermService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractTermService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractTermService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractTermService.CacheData.Data, this.leaseContractTermService.CacheData.TotalRecords);
    }
  }
