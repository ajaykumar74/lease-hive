...
@Component({
  ...
  standalone: false,
  templateUrl: './contractNotice-list.component.html'
})
export class ContractNoticeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractNoticeService.CacheData.IsLoaded) {
      this.currentPage = this.contractNoticeService.CacheData.CurrentPage;
      this.objSearch = this.contractNoticeService.CacheData.objSearch;
      this.permission = this.contractNoticeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractNoticeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractNoticeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractNoticeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractNoticeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractNoticeService.CacheData.Data, this.contractNoticeService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractNotices/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractNotices/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractNotices/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
