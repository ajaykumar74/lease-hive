...
@Component({
  ...
  standalone: false,
  templateUrl: './contractObligation-list.component.html'
})
export class ContractObligationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractObligationService.CacheData.IsLoaded) {
      this.currentPage = this.contractObligationService.CacheData.CurrentPage;
      this.objSearch = this.contractObligationService.CacheData.objSearch;
      this.permission = this.contractObligationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractObligationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractObligationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractObligationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractObligationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractObligationService.CacheData.Data, this.contractObligationService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractObligations/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractObligations/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractObligations/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
