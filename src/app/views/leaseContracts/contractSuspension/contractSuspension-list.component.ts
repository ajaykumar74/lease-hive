...
@Component({
  ...
  standalone: false,
  templateUrl: './contractSuspension-list.component.html'
})
export class ContractSuspensionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractSuspensionService.CacheData.IsLoaded) {
      this.currentPage = this.contractSuspensionService.CacheData.CurrentPage;
      this.objSearch = this.contractSuspensionService.CacheData.objSearch;
      this.permission = this.contractSuspensionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractSuspensionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractSuspensionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractSuspensionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractSuspensionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractSuspensionService.CacheData.Data, this.contractSuspensionService.CacheData.TotalRecords);
    }
  }
