...
@Component({
  ...
  standalone: false,
  templateUrl: './contractCondition-list.component.html'
})
export class ContractConditionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractConditionService.CacheData.IsLoaded) {
      this.currentPage = this.contractConditionService.CacheData.CurrentPage;
      this.objSearch = this.contractConditionService.CacheData.objSearch;
      this.permission = this.contractConditionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractConditionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractConditionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractConditionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractConditionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractConditionService.CacheData.Data, this.contractConditionService.CacheData.TotalRecords);
    }
  }
