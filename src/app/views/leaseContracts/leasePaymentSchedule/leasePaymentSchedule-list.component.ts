...
@Component({
  ...
  standalone: false,
  templateUrl: './leasePaymentSchedule-list.component.html'
})
export class LeasePaymentScheduleListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leasePaymentScheduleService.CacheData.IsLoaded) {
      this.currentPage = this.leasePaymentScheduleService.CacheData.CurrentPage;
      this.objSearch = this.leasePaymentScheduleService.CacheData.objSearch;
      this.permission = this.leasePaymentScheduleService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leasePaymentScheduleService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leasePaymentScheduleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leasePaymentScheduleService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leasePaymentScheduleService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leasePaymentScheduleService.CacheData.Data, this.leasePaymentScheduleService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/leasePaymentSchedules/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/leasePaymentSchedules/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/leasePaymentSchedules/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
