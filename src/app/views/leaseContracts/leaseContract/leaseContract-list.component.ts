...
@Component({
  ...
  standalone: false,
  templateUrl: './leaseContract-list.component.html'
})
export class LeaseContractListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.leaseContractService.CacheData.IsLoaded) {
      this.currentPage = this.leaseContractService.CacheData.CurrentPage;
      this.objSearch = this.leaseContractService.CacheData.objSearch;
      this.permission = this.leaseContractService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.leaseContractService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.leaseContractService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.leaseContractService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.leaseContractService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.leaseContractService.CacheData.Data, this.leaseContractService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/leaseContracts/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/leaseContracts/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/leaseContracts/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
