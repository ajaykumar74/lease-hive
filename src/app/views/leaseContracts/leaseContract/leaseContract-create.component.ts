...
@Component({
  selector: 'app-leaseContract-create',
  standalone: false,
  templateUrl: './leaseContract-create.component.html' ,
   providers: [ MessageService]
})
export class LeaseContractCreateComponent implements OnInit {
...
RecordStatus: obj.RecordStatus || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/leaseContracts/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
RecordStatus: formValues.RecordStatus || null,
	  ...
	  this.spinner.show(); 
    this.leaseContractService.create(createdObj).subscribe({
      next: data => {	   
