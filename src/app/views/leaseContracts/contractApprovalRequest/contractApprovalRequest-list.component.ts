...
@Component({
  ...
  standalone: false,
  templateUrl: './contractApprovalRequest-list.component.html'
})
export class ContractApprovalRequestListComponent implements OnInit {
  ...
  lstMain: IContractApprovalRequest[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractApprovalRequestService.CacheData.IsLoaded) {
      this.currentPage = this.contractApprovalRequestService.CacheData.CurrentPage;
      this.objSearch = this.contractApprovalRequestService.CacheData.objSearch;
      this.permission = this.contractApprovalRequestService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractApprovalRequestService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractApprovalRequestService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractApprovalRequestService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractApprovalRequestService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractApprovalRequestService.CacheData.Data, this.contractApprovalRequestService.CacheData.TotalRecords);
    }
  }
