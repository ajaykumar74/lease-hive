...
@Component({
  ...
  standalone: false,
  templateUrl: './contractAssetReturnInstruction-list.component.html'
})
export class ContractAssetReturnInstructionListComponent implements OnInit {
  ...
  lstMain: IContractAssetReturnInstruction[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractAssetReturnInstructionService.CacheData.IsLoaded) {
      this.currentPage = this.contractAssetReturnInstructionService.CacheData.CurrentPage;
      this.objSearch = this.contractAssetReturnInstructionService.CacheData.objSearch;
      this.permission = this.contractAssetReturnInstructionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractAssetReturnInstructionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractAssetReturnInstructionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractAssetReturnInstructionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractAssetReturnInstructionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractAssetReturnInstructionService.CacheData.Data, this.contractAssetReturnInstructionService.CacheData.TotalRecords);
    }
  }
