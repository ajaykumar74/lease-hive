...
@Component({
  ...
  standalone: false,
  templateUrl: './contractApprovalAction-list.component.html'
})
export class ContractApprovalActionListComponent implements OnInit {
  ...
  lstMain: IContractApprovalAction[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractApprovalActionService.CacheData.IsLoaded) {
      this.currentPage = this.contractApprovalActionService.CacheData.CurrentPage;
      this.objSearch = this.contractApprovalActionService.CacheData.objSearch;
      this.permission = this.contractApprovalActionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractApprovalActionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractApprovalActionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractApprovalActionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractApprovalActionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractApprovalActionService.CacheData.Data, this.contractApprovalActionService.CacheData.TotalRecords);
    }
  }
