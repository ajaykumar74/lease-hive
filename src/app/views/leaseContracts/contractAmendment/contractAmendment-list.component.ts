...
@Component({
  ...
  standalone: false,
  templateUrl: './contractAmendment-list.component.html'
})
export class ContractAmendmentListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.contractAmendmentService.CacheData.IsLoaded) {
      this.currentPage = this.contractAmendmentService.CacheData.CurrentPage;
      this.objSearch = this.contractAmendmentService.CacheData.objSearch;
      this.permission = this.contractAmendmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractAmendmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractAmendmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractAmendmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractAmendmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractAmendmentService.CacheData.Data, this.contractAmendmentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractAmendments/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractAmendments/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractAmendments/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
