...
@Component({
  ...
  standalone: false,
  templateUrl: './contractConditionEvidence-list.component.html'
})
export class ContractConditionEvidenceListComponent implements OnInit {
  ...
  lstMain: IContractConditionEvidence[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.contractConditionEvidenceService.CacheData.IsLoaded) {
      this.currentPage = this.contractConditionEvidenceService.CacheData.CurrentPage;
      this.objSearch = this.contractConditionEvidenceService.CacheData.objSearch;
      this.permission = this.contractConditionEvidenceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.contractConditionEvidenceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.contractConditionEvidenceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.contractConditionEvidenceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.contractConditionEvidenceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.contractConditionEvidenceService.CacheData.Data, this.contractConditionEvidenceService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/contractConditionEvidences/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/contractConditionEvidences/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/contractConditionEvidences/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
