...
@Component({
  ...
  standalone: false,
  templateUrl: './assetUser-list.component.html'
})
export class AssetUserListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetUserService.CacheData.IsLoaded) {
      this.currentPage = this.assetUserService.CacheData.CurrentPage;
      this.objSearch = this.assetUserService.CacheData.objSearch;
      this.permission = this.assetUserService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetUserService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetUserService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetUserService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetUserService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetUserService.CacheData.Data, this.assetUserService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/assetUsers/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/assetUsers/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/assetUsers/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
