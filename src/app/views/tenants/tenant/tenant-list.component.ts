...
export class TenantListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.tenantService.CacheData.IsLoaded) {
      this.currentPage = this.tenantService.CacheData.CurrentPage;
      this.objSearch = this.tenantService.CacheData.objSearch;
      this.permission = this.tenantService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.tenantService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.tenantService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.tenantService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.tenantService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.tenantService.CacheData.Data, this.tenantService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/tenants/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/tenants/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/tenants/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
