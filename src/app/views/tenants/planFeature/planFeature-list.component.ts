...
@Component({
  ...
  standalone: false,
  templateUrl: './planFeature-list.component.html'
})
export class PlanFeatureListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.planFeatureService.CacheData.IsLoaded) {
      this.currentPage = this.planFeatureService.CacheData.CurrentPage;
      this.objSearch = this.planFeatureService.CacheData.objSearch;
      this.permission = this.planFeatureService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.planFeatureService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.planFeatureService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.planFeatureService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.planFeatureService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.planFeatureService.CacheData.Data, this.planFeatureService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/planFeatures/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/planFeatures/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/planFeatures/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
