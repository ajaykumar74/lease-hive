...
@Component({
  ...
  standalone: false,
  templateUrl: './customerDepartment-list.component.html'
})
export class CustomerDepartmentListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerDepartmentService.CacheData.IsLoaded) {
      this.currentPage = this.customerDepartmentService.CacheData.CurrentPage;
      this.objSearch = this.customerDepartmentService.CacheData.objSearch;
      this.permission = this.customerDepartmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerDepartmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.customerDepartmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.customerDepartmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.customerDepartmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.customerDepartmentService.CacheData.Data, this.customerDepartmentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/customerDepartments/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/customerDepartments/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/customerDepartments/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
