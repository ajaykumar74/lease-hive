...
@Component({
  ...
  standalone: false,
  templateUrl: './userDelegation-list.component.html'
})
export class UserDelegationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.userDelegationService.CacheData.IsLoaded) {
      this.currentPage = this.userDelegationService.CacheData.CurrentPage;
      this.objSearch = this.userDelegationService.CacheData.objSearch;
      this.permission = this.userDelegationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.userDelegationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.userDelegationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.userDelegationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.userDelegationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.userDelegationService.CacheData.Data, this.userDelegationService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/userDelegations/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/userDelegations/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/userDelegations/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
