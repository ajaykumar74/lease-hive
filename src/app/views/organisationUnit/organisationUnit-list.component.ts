...
@Component({
  ...
  standalone: false,
  templateUrl: './organisationUnit-list.component.html'
})
export class OrganisationUnitListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.organisationUnitService.CacheData.IsLoaded) {
      this.currentPage = this.organisationUnitService.CacheData.CurrentPage;
      this.objSearch = this.organisationUnitService.CacheData.objSearch;
      this.permission = this.organisationUnitService.CacheData.permission;
    }  
  }
  ...
  pageChanged(event: { first?: number; rows?: number; page?: number; pageCount?: number }): void {
    ...
    if (isReload || this.organisationUnitService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.organisationUnitService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.organisationUnitService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.organisationUnitService.CacheData.Data, this.organisationUnitService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'UnitName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'UnitCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/organisations/units/edit', obj.Id]);
    }
    else {
        this.router.navigate(['/business/organisations/units/view', obj.Id]);
    } 
  
  };

  onViewClick(item: IOrganisationUnit): void { this.router.navigate(['/business/organisations/units/view', item.Id], { state: { organisationUnit: item } }); }
  onEditClick(item: IOrganisationUnit): void { this.router.navigate(['/business/organisations/units/edit', item.Id], { state: { organisationUnit: item } }); }
  getInitials(item: IOrganisationUnit): string { return (item.UnitName || item.UnitCode || 'Unit').split(/\s+/).filter(Boolean).slice(0, 2).map(x => x[0]).join('').toUpperCase(); }
