...
@Component({
    templateUrl: './organisationUnit-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class OrganisationUnitViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    organisationUnit: IOrganisationUnit = {} as IOrganisationUnit;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/organisationUnit/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
