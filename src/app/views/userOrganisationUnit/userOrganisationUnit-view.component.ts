...
@Component({
    templateUrl: './userOrganisationUnit-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class UserOrganisationUnitViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    userOrganisationUnit: IUserOrganisationUnit = {} as IUserOrganisationUnit;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/userOrganisationUnit/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
