...
@Component({
  ...
  standalone: false,
  templateUrl: './rFQSupplier-list.component.html'
})
export class RFQSupplierListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.rFQSupplierService.CacheData.IsLoaded) {
      this.currentPage = this.rFQSupplierService.CacheData.CurrentPage;
      this.objSearch = this.rFQSupplierService.CacheData.objSearch;
      this.permission = this.rFQSupplierService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.rFQSupplierService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.rFQSupplierService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.rFQSupplierService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.rFQSupplierService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.rFQSupplierService.CacheData.Data, this.rFQSupplierService.CacheData.TotalRecords);
    }
  }
