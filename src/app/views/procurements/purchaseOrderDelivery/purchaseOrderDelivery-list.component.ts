...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseOrderDelivery-list.component.html'
})
export class PurchaseOrderDeliveryListComponent implements OnInit {
  ...
  lstMain: IPurchaseOrderDelivery[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.purchaseOrderDeliveryService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseOrderDeliveryService.CacheData.CurrentPage;
      this.objSearch = this.purchaseOrderDeliveryService.CacheData.objSearch;
      this.permission = this.purchaseOrderDeliveryService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseOrderDeliveryService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseOrderDeliveryService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseOrderDeliveryService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseOrderDeliveryService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseOrderDeliveryService.CacheData.Data, this.purchaseOrderDeliveryService.CacheData.TotalRecords);
    }
  }
