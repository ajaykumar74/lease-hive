...
@Component({
    templateUrl: './invoiceMatch-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class InvoiceMatchViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    invoiceMatch: IInvoiceMatch = {} as IInvoiceMatch;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/invoiceMatch/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
