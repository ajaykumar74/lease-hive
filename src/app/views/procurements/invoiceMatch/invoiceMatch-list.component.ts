...
@Component({
  ...
  standalone: false,
  templateUrl: './invoiceMatch-list.component.html'
})
export class InvoiceMatchListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.invoiceMatchService.CacheData.IsLoaded) {
      this.currentPage = this.invoiceMatchService.CacheData.CurrentPage;
      this.objSearch = this.invoiceMatchService.CacheData.objSearch;
      this.permission = this.invoiceMatchService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.invoiceMatchService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.invoiceMatchService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.invoiceMatchService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.invoiceMatchService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.invoiceMatchService.CacheData.Data, this.invoiceMatchService.CacheData.TotalRecords);
    }
  }
