...
@Component({
  selector: 'app-purchaseRequisitionStatus-edit',
  standalone: false,
  templateUrl: './purchaseRequisitionStatus-edit.component.html',
  providers: [ MessageService]
})
export class PurchaseRequisitionStatusEditComponent implements OnInit {
...
EffectiveTo: new FormControl(new Date(), [Validators.required]),
  ...
  loadUI(): void {
    ...
    this.purchaseRequisitionStatusService.getById(this.selectedId).subscribe({
      next: data => {	        
        this.purchaseRequisitionStatus = data.data;
		this.permission = data.permission;
        this.objMaster = { ...this.purchaseRequisitionStatus };
        this.populateUI(this.purchaseRequisitionStatus);
      },
...
EffectiveTo:  obj.EffectiveTo || new Date(),
	 ...
	 this.Caption = "PurchaseRequisitionStatus Details #" + obj.Id;
  } 

  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/purchaseRequisitionStatus/create', { id: -1 }]);
    }
    else if (key == "Save") {
      this.Save();
    }
...
EffectiveTo:  formValues.EffectiveTo || null,
	...
	this.spinner.show();  	   
    this.purchaseRequisitionStatusService.update(this.purchaseRequisitionStatus.Id, updatedObj).subscribe({
      next: data => {
