...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseRequisitionStatus-list.component.html'
})
export class PurchaseRequisitionStatusListComponent implements OnInit {
  ...
  lstMain: IPurchaseRequisitionStatus[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.purchaseRequisitionStatusService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseRequisitionStatusService.CacheData.CurrentPage;
      this.objSearch = this.purchaseRequisitionStatusService.CacheData.objSearch;
      this.permission = this.purchaseRequisitionStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseRequisitionStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseRequisitionStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseRequisitionStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseRequisitionStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseRequisitionStatusService.CacheData.Data, this.purchaseRequisitionStatusService.CacheData.TotalRecords);
    }
  }
