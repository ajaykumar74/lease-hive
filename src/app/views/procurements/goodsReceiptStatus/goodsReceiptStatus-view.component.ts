...
@Component({
    templateUrl: './goodsReceiptStatus-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class GoodsReceiptStatusViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    goodsReceiptStatus: IGoodsReceiptStatus = {} as IGoodsReceiptStatus;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/goodsReceiptStatus/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
