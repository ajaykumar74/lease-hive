...
export class GoodsReceiptListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.goodsReceiptService.CacheData.IsLoaded) {
      this.currentPage = this.goodsReceiptService.CacheData.CurrentPage;
      this.objSearch = this.goodsReceiptService.CacheData.objSearch;
      this.permission = this.goodsReceiptService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.goodsReceiptService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.goodsReceiptService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.goodsReceiptService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.goodsReceiptService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.goodsReceiptService.CacheData.Data, this.goodsReceiptService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/goodsReceipts/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/goodsReceipts/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/goodsReceipts/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
