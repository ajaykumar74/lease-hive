...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseOrderStatus-list.component.html'
})
export class PurchaseOrderStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.purchaseOrderStatusService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseOrderStatusService.CacheData.CurrentPage;
      this.objSearch = this.purchaseOrderStatusService.CacheData.objSearch;
      this.permission = this.purchaseOrderStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseOrderStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseOrderStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseOrderStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseOrderStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseOrderStatusService.CacheData.Data, this.purchaseOrderStatusService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/purchaseOrderStatuss/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/purchaseOrderStatuss/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/purchaseOrderStatuss/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
