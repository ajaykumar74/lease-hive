...
@Component({
  ...
  standalone: false,
  templateUrl: './procurementException-list.component.html'
})
export class ProcurementExceptionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.procurementExceptionService.CacheData.IsLoaded) {
      this.currentPage = this.procurementExceptionService.CacheData.CurrentPage;
      this.objSearch = this.procurementExceptionService.CacheData.objSearch;
      this.permission = this.procurementExceptionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.procurementExceptionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.procurementExceptionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.procurementExceptionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.procurementExceptionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.procurementExceptionService.CacheData.Data, this.procurementExceptionService.CacheData.TotalRecords);
    }
  }
