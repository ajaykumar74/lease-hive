...
export class RFQListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.rFQService.CacheData.IsLoaded) {
      this.currentPage = this.rFQService.CacheData.CurrentPage;
      this.objSearch = this.rFQService.CacheData.objSearch;
      this.permission = this.rFQService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.rFQService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.rFQService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.rFQService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.rFQService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.rFQService.CacheData.Data, this.rFQService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/procurement/rfqs/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/procurement/rfqs/view/' + obj.Id]);
    } 
