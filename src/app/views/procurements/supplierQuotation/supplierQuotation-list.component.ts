...
@Component({
  ...
  standalone: false,
  templateUrl: './supplierQuotation-list.component.html'
})
export class SupplierQuotationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.supplierQuotationService.CacheData.IsLoaded) {
      this.currentPage = this.supplierQuotationService.CacheData.CurrentPage;
      this.objSearch = this.supplierQuotationService.CacheData.objSearch;
      this.permission = this.supplierQuotationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.supplierQuotationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.supplierQuotationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.supplierQuotationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.supplierQuotationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.supplierQuotationService.CacheData.Data, this.supplierQuotationService.CacheData.TotalRecords);
    }
  }
