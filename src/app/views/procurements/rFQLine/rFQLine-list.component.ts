...
@Component({
  ...
  standalone: false,
  templateUrl: './rFQLine-list.component.html'
})
export class RFQLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.rFQLineService.CacheData.IsLoaded) {
      this.currentPage = this.rFQLineService.CacheData.CurrentPage;
      this.objSearch = this.rFQLineService.CacheData.objSearch;
      this.permission = this.rFQLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.rFQLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.rFQLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.rFQLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.rFQLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.rFQLineService.CacheData.Data, this.rFQLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/rFQLines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/rFQLines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/rFQLines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
