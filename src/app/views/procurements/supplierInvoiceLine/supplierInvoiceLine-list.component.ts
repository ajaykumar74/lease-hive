...
@Component({
  ...
  standalone: false,
  templateUrl: './supplierInvoiceLine-list.component.html'
})
export class SupplierInvoiceLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.supplierInvoiceLineService.CacheData.IsLoaded) {
      this.currentPage = this.supplierInvoiceLineService.CacheData.CurrentPage;
      this.objSearch = this.supplierInvoiceLineService.CacheData.objSearch;
      this.permission = this.supplierInvoiceLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.supplierInvoiceLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.supplierInvoiceLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.supplierInvoiceLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.supplierInvoiceLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.supplierInvoiceLineService.CacheData.Data, this.supplierInvoiceLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/supplierInvoiceLines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/supplierInvoiceLines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/supplierInvoiceLines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
