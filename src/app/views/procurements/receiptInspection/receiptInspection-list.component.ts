...
@Component({
  ...
  standalone: false,
  templateUrl: './receiptInspection-list.component.html'
})
export class ReceiptInspectionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.receiptInspectionService.CacheData.IsLoaded) {
      this.currentPage = this.receiptInspectionService.CacheData.CurrentPage;
      this.objSearch = this.receiptInspectionService.CacheData.objSearch;
      this.permission = this.receiptInspectionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.receiptInspectionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.receiptInspectionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.receiptInspectionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.receiptInspectionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.receiptInspectionService.CacheData.Data, this.receiptInspectionService.CacheData.TotalRecords);
    }
  }
