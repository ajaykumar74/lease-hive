...
@Component({
  ...
  standalone: false,
  templateUrl: './goodsReceiptSerial-list.component.html'
})
export class GoodsReceiptSerialListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.goodsReceiptSerialService.CacheData.IsLoaded) {
      this.currentPage = this.goodsReceiptSerialService.CacheData.CurrentPage;
      this.objSearch = this.goodsReceiptSerialService.CacheData.objSearch;
      this.permission = this.goodsReceiptSerialService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.goodsReceiptSerialService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.goodsReceiptSerialService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.goodsReceiptSerialService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.goodsReceiptSerialService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.goodsReceiptSerialService.CacheData.Data, this.goodsReceiptSerialService.CacheData.TotalRecords);
    }
  }
