...
@Component({
  ...
  standalone: false,
  templateUrl: './procurementDocumentLink-list.component.html'
})
export class ProcurementDocumentLinkListComponent implements OnInit {
  ...
  lstMain: IProcurementDocumentLink[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.procurementDocumentLinkService.CacheData.IsLoaded) {
      this.currentPage = this.procurementDocumentLinkService.CacheData.CurrentPage;
      this.objSearch = this.procurementDocumentLinkService.CacheData.objSearch;
      this.permission = this.procurementDocumentLinkService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.procurementDocumentLinkService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.procurementDocumentLinkService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.procurementDocumentLinkService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.procurementDocumentLinkService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.procurementDocumentLinkService.CacheData.Data, this.procurementDocumentLinkService.CacheData.TotalRecords);
    }
  }
