...
@Component({
  ...
  standalone: false,
  templateUrl: './supplierQuotationLine-list.component.html'
})
export class SupplierQuotationLineListComponent implements OnInit {
  ...
  lstMain: ISupplierQuotationLine[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.supplierQuotationLineService.CacheData.IsLoaded) {
      this.currentPage = this.supplierQuotationLineService.CacheData.CurrentPage;
      this.objSearch = this.supplierQuotationLineService.CacheData.objSearch;
      this.permission = this.supplierQuotationLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.supplierQuotationLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.supplierQuotationLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.supplierQuotationLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.supplierQuotationLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.supplierQuotationLineService.CacheData.Data, this.supplierQuotationLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/supplierQuotationLines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/supplierQuotationLines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/supplierQuotationLines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
