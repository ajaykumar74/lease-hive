...
@Component({
  ...
  standalone: false,
  templateUrl: './supplierInvoice-list.component.html'
})
export class SupplierInvoiceListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.supplierInvoiceService.CacheData.IsLoaded) {
      this.currentPage = this.supplierInvoiceService.CacheData.CurrentPage;
      this.objSearch = this.supplierInvoiceService.CacheData.objSearch;
      this.permission = this.supplierInvoiceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.supplierInvoiceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.supplierInvoiceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.supplierInvoiceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.supplierInvoiceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.supplierInvoiceService.CacheData.Data, this.supplierInvoiceService.CacheData.TotalRecords);
    }
  }
