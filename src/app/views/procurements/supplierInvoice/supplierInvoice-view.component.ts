...
@Component({
    templateUrl: './supplierInvoice-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class SupplierInvoiceViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    supplierInvoice: ISupplierInvoice = {} as ISupplierInvoice;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/supplierInvoice/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
