...
CapturedBy: obj.CapturedBy || 0,
  ...
  addLineItem(lineItem?: Partial<ISupplierInvoiceLine>): void {
    const line = this.fb.group({
      ...
      SupplierInvoiceId: new FormControl(lineItem?.SupplierInvoiceId || 0), LineNo: new FormControl(lineItem?.LineNo || this.lineItems.length + 1),
      ...
      InvoicedQuantity: new FormControl(lineItem?.InvoicedQuantity || 1, [Validators.required, Validators.min(1)]),
...
LineItems: this.lineItems.getRawValue().map((line: any) => ({ ...line, SupplierInvoiceId: 0 })),
RecordStatus: 'Active',
    } as ISupplierInvoice ; 
