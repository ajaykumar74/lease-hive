...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseRequisition-list.component.html'
})
export class PurchaseRequisitionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.purchaseRequisitionService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseRequisitionService.CacheData.CurrentPage;
      this.objSearch = this.purchaseRequisitionService.CacheData.objSearch;
      this.permission = this.purchaseRequisitionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseRequisitionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseRequisitionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseRequisitionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseRequisitionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseRequisitionService.CacheData.Data, this.purchaseRequisitionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/purchaseRequisitions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/purchaseRequisitions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/purchaseRequisitions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
