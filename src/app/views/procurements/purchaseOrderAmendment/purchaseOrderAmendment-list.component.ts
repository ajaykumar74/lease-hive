...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseOrderAmendment-list.component.html'
})
export class PurchaseOrderAmendmentListComponent implements OnInit {
  ...
  lstMain: IPurchaseOrderAmendment[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.purchaseOrderAmendmentService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseOrderAmendmentService.CacheData.CurrentPage;
      this.objSearch = this.purchaseOrderAmendmentService.CacheData.objSearch;
      this.permission = this.purchaseOrderAmendmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseOrderAmendmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseOrderAmendmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseOrderAmendmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseOrderAmendmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseOrderAmendmentService.CacheData.Data, this.purchaseOrderAmendmentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/purchaseOrderAmendments/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/purchaseOrderAmendments/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/purchaseOrderAmendments/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
