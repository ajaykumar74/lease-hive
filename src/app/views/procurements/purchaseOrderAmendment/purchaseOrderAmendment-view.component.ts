...
@Component({
    templateUrl: './purchaseOrderAmendment-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class PurchaseOrderAmendmentViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    purchaseOrderAmendment: IPurchaseOrderAmendment = {} as IPurchaseOrderAmendment;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/purchaseOrderAmendment/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
