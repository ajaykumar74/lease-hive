...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseRequisitionLine-list.component.html'
})
export class PurchaseRequisitionLineListComponent implements OnInit {
  ...
  lstMain: IPurchaseRequisitionLine[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.purchaseRequisitionLineService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseRequisitionLineService.CacheData.CurrentPage;
      this.objSearch = this.purchaseRequisitionLineService.CacheData.objSearch;
      this.permission = this.purchaseRequisitionLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseRequisitionLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseRequisitionLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseRequisitionLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseRequisitionLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseRequisitionLineService.CacheData.Data, this.purchaseRequisitionLineService.CacheData.TotalRecords);
    }
  }
