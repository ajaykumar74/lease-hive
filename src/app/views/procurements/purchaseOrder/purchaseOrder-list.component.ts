...
export class PurchaseOrderListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.purchaseOrderService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseOrderService.CacheData.CurrentPage;
      this.objSearch = this.purchaseOrderService.CacheData.objSearch;
      this.permission = this.purchaseOrderService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseOrderService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseOrderService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseOrderService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseOrderService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseOrderService.CacheData.Data, this.purchaseOrderService.CacheData.TotalRecords);
    }
  }
