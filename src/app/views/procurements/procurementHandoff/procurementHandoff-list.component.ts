...
@Component({
  ...
  standalone: false,
  templateUrl: './procurementHandoff-list.component.html'
})
export class ProcurementHandoffListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.procurementHandoffService.CacheData.IsLoaded) {
      this.currentPage = this.procurementHandoffService.CacheData.CurrentPage;
      this.objSearch = this.procurementHandoffService.CacheData.objSearch;
      this.permission = this.procurementHandoffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.procurementHandoffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.procurementHandoffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.procurementHandoffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.procurementHandoffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.procurementHandoffService.CacheData.Data, this.procurementHandoffService.CacheData.TotalRecords);
    }
  }
