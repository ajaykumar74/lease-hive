...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseOrderLine-list.component.html'
})
export class PurchaseOrderLineListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.purchaseOrderLineService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseOrderLineService.CacheData.CurrentPage;
      this.objSearch = this.purchaseOrderLineService.CacheData.objSearch;
      this.permission = this.purchaseOrderLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseOrderLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseOrderLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseOrderLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseOrderLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseOrderLineService.CacheData.Data, this.purchaseOrderLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/purchaseOrderLines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/purchaseOrderLines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/purchaseOrderLines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
