...
@Component({
  selector: 'app-purchaseOrderLine-create',
  standalone: false,
  templateUrl: './purchaseOrderLine-create.component.html' ,
   providers: [ MessageService]
})
export class PurchaseOrderLineCreateComponent implements OnInit {
...
SpecificationsJson: new FormControl('', [Validators.maxLength(8000), ]), 
    ...
    this.Caption = 'Create PurchaseOrderLine';
...
SpecificationsJson: formValues.SpecificationsJson || null,
	  ...
	  this.spinner.show(); 
    this.purchaseOrderLineService.create(createdObj).subscribe({
      next: data => {	   
