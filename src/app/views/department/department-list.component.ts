...
export class DepartmentListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.departmentService.CacheData.IsLoaded) {
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.departmentService.CacheData.IsLoaded);
    }, 500);
  }
    ...
    if (isReload || this.departmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.departmentService.search(searchParam).subscribe({
        next: res => {
          ...
          this.departmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/departments/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/departments/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/departments/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
