...
@Component({
  ...
  standalone: false,
  templateUrl: './userPartyAccess-list.component.html'
})
export class UserPartyAccessListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.userPartyAccessService.CacheData.IsLoaded) {
      this.currentPage = this.userPartyAccessService.CacheData.CurrentPage;
      this.objSearch = this.userPartyAccessService.CacheData.objSearch;
      this.permission = this.userPartyAccessService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.userPartyAccessService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.userPartyAccessService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.userPartyAccessService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.userPartyAccessService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.userPartyAccessService.CacheData.Data, this.userPartyAccessService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'ApplicationUserId', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/userPartyAccesss/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/userPartyAccesss/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/userPartyAccesss/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
