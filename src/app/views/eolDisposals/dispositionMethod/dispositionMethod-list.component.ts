...
@Component({
  ...
  standalone: false,
  templateUrl: './dispositionMethod-list.component.html'
})
export class DispositionMethodListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.dispositionMethodService.CacheData.IsLoaded) {
      this.currentPage = this.dispositionMethodService.CacheData.CurrentPage;
      this.objSearch = this.dispositionMethodService.CacheData.objSearch;
      this.permission = this.dispositionMethodService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.dispositionMethodService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.dispositionMethodService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.dispositionMethodService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.dispositionMethodService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.dispositionMethodService.CacheData.Data, this.dispositionMethodService.CacheData.TotalRecords);
    }
  }
