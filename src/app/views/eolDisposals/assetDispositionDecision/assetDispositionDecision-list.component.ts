...
@Component({
  ...
  standalone: false,
  templateUrl: './assetDispositionDecision-list.component.html'
})
export class AssetDispositionDecisionListComponent implements OnInit {
  ...
  lstMain: IAssetDispositionDecision[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.assetDispositionDecisionService.CacheData.IsLoaded) {
      this.currentPage = this.assetDispositionDecisionService.CacheData.CurrentPage;
      this.objSearch = this.assetDispositionDecisionService.CacheData.objSearch;
      this.permission = this.assetDispositionDecisionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetDispositionDecisionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetDispositionDecisionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetDispositionDecisionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetDispositionDecisionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetDispositionDecisionService.CacheData.Data, this.assetDispositionDecisionService.CacheData.TotalRecords);
    }
  }
