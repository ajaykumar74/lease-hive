...
@Component({
  ...
  standalone: false,
  templateUrl: './disposalBid-list.component.html'
})
export class DisposalBidListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.disposalBidService.CacheData.IsLoaded) {
      this.currentPage = this.disposalBidService.CacheData.CurrentPage;
      this.objSearch = this.disposalBidService.CacheData.objSearch;
      this.permission = this.disposalBidService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.disposalBidService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.disposalBidService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.disposalBidService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.disposalBidService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.disposalBidService.CacheData.Data, this.disposalBidService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/eol-disposal/disposition/market/bids/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/eol-disposal/disposition/market/bids/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/eol-disposal/disposition/market/bids/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
