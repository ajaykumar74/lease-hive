...
@Component({
  ...
  standalone: false,
  templateUrl: './disposalAuction-list.component.html'
})
export class DisposalAuctionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.disposalAuctionService.CacheData.IsLoaded) {
      this.currentPage = this.disposalAuctionService.CacheData.CurrentPage;
      this.objSearch = this.disposalAuctionService.CacheData.objSearch;
      this.permission = this.disposalAuctionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.disposalAuctionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.disposalAuctionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.disposalAuctionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.disposalAuctionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.disposalAuctionService.CacheData.Data, this.disposalAuctionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/eol-disposal/disposition/market/auctions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/eol-disposal/disposition/market/auctions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/eol-disposal/disposition/market/auctions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
