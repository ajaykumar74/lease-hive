...
@Component({
    templateUrl: './endOfLeaseReason-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class EndOfLeaseReasonViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    endOfLeaseReason: IEndOfLeaseReason = {} as IEndOfLeaseReason;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/endOfLeaseReason/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
