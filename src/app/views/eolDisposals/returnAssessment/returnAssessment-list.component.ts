...
@Component({
  ...
  standalone: false,
  templateUrl: './returnAssessment-list.component.html'
})
export class ReturnAssessmentListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.returnAssessmentService.CacheData.IsLoaded) {
      this.currentPage = this.returnAssessmentService.CacheData.CurrentPage;
      this.objSearch = this.returnAssessmentService.CacheData.objSearch;
      this.permission = this.returnAssessmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.returnAssessmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.returnAssessmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.returnAssessmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.returnAssessmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.returnAssessmentService.CacheData.Data, this.returnAssessmentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/returnAssessments/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/returnAssessments/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/returnAssessments/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
