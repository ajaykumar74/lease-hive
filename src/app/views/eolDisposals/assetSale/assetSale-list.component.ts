...
@Component({
  ...
  standalone: false,
  templateUrl: './assetSale-list.component.html'
})
export class AssetSaleListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetSaleService.CacheData.IsLoaded) {
      this.currentPage = this.assetSaleService.CacheData.CurrentPage;
      this.objSearch = this.assetSaleService.CacheData.objSearch;
      this.permission = this.assetSaleService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetSaleService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetSaleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetSaleService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetSaleService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetSaleService.CacheData.Data, this.assetSaleService.CacheData.TotalRecords);
    }
  }
