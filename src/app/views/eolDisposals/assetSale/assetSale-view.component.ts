...
@Component({
    templateUrl: './assetSale-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetSaleViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetSale: IAssetSale = {} as IAssetSale;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetSale/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
