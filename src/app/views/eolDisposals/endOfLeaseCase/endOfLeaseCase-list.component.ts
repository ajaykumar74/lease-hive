...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseCase-list.component.html'
})
export class EndOfLeaseCaseListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.endOfLeaseCaseService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseCaseService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseCaseService.CacheData.objSearch;
      this.permission = this.endOfLeaseCaseService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseCaseService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseCaseService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseCaseService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseCaseService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseCaseService.CacheData.Data, this.endOfLeaseCaseService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/endOfLeaseCases/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/endOfLeaseCases/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/endOfLeaseCases/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
