...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseStatus-list.component.html'
})
export class EndOfLeaseStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.endOfLeaseStatusService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseStatusService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseStatusService.CacheData.objSearch;
      this.permission = this.endOfLeaseStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseStatusService.CacheData.Data, this.endOfLeaseStatusService.CacheData.TotalRecords);
    }
  }
