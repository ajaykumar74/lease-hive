...
@Component({
    templateUrl: './endOfLeaseStatus-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class EndOfLeaseStatusViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    endOfLeaseStatus: IEndOfLeaseStatus = {} as IEndOfLeaseStatus;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/endOfLeaseStatus/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
