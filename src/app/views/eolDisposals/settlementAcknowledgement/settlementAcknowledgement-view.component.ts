...
@Component({
    templateUrl: './settlementAcknowledgement-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class SettlementAcknowledgementViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    settlementAcknowledgement: ISettlementAcknowledgement = {} as ISettlementAcknowledgement;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/settlementAcknowledgement/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
