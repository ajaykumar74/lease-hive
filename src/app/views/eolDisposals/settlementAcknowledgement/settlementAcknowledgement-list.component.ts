...
@Component({
  ...
  standalone: false,
  templateUrl: './settlementAcknowledgement-list.component.html'
})
export class SettlementAcknowledgementListComponent implements OnInit {
  ...
  lstMain: ISettlementAcknowledgement[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.settlementAcknowledgementService.CacheData.IsLoaded) {
      this.currentPage = this.settlementAcknowledgementService.CacheData.CurrentPage;
      this.objSearch = this.settlementAcknowledgementService.CacheData.objSearch;
      this.permission = this.settlementAcknowledgementService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.settlementAcknowledgementService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.settlementAcknowledgementService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.settlementAcknowledgementService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.settlementAcknowledgementService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.settlementAcknowledgementService.CacheData.Data, this.settlementAcknowledgementService.CacheData.TotalRecords);
    }
  }
