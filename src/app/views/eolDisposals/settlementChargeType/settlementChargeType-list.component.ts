...
@Component({
  ...
  standalone: false,
  templateUrl: './settlementChargeType-list.component.html'
})
export class SettlementChargeTypeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.settlementChargeTypeService.CacheData.IsLoaded) {
      this.currentPage = this.settlementChargeTypeService.CacheData.CurrentPage;
      this.objSearch = this.settlementChargeTypeService.CacheData.objSearch;
      this.permission = this.settlementChargeTypeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.settlementChargeTypeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.settlementChargeTypeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.settlementChargeTypeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.settlementChargeTypeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.settlementChargeTypeService.CacheData.Data, this.settlementChargeTypeService.CacheData.TotalRecords);
    }
  }
