...
@Component({
  ...
  standalone: false,
  templateUrl: './disposalAward-list.component.html'
})
export class DisposalAwardListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.disposalAwardService.CacheData.IsLoaded) {
      this.currentPage = this.disposalAwardService.CacheData.CurrentPage;
      this.objSearch = this.disposalAwardService.CacheData.objSearch;
      this.permission = this.disposalAwardService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.disposalAwardService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.disposalAwardService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.disposalAwardService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.disposalAwardService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.disposalAwardService.CacheData.Data, this.disposalAwardService.CacheData.TotalRecords);
    }
  }
