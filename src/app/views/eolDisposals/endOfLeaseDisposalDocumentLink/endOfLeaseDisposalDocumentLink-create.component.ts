...
@Component({
  selector: 'app-endOfLeaseDisposalDocumentLink-create',
  standalone: false,
  templateUrl: './endOfLeaseDisposalDocumentLink-create.component.html' ,
   providers: [ MessageService]
})
export class EndOfLeaseDisposalDocumentLinkCreateComponent implements OnInit {
...
this.recordstatusOptions.push({Text: 'Archived', Value: 'Archived' });
 ...
 loadUI(): void {
    ...
    this.endOfLeaseDisposalDocumentLinkService.getById(this.selectedId).subscribe({
      next: data => {
        this.endOfLeaseDisposalDocumentLink = data;
        this.objMaster = { ...this.endOfLeaseDisposalDocumentLink };
...
RecordStatus: obj.RecordStatus || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/endOfLeaseDisposalDocumentLinks/create']);
    }
    else if (key == "Save") {
      this.Save();
    }
...
RecordStatus: formValues.RecordStatus || null,
	  ...
	  this.spinner.show(); 
    this.endOfLeaseDisposalDocumentLinkService.create(createdObj).subscribe({
      next: data => {	   
