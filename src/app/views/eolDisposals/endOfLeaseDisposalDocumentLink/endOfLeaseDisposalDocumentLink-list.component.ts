...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseDisposalDocumentLink-list.component.html'
})
export class EndOfLeaseDisposalDocumentLinkListComponent implements OnInit {
  ...
  lstMain: IEndOfLeaseDisposalDocumentLink[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.endOfLeaseDisposalDocumentLinkService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseDisposalDocumentLinkService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseDisposalDocumentLinkService.CacheData.objSearch;
      this.permission = this.endOfLeaseDisposalDocumentLinkService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseDisposalDocumentLinkService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseDisposalDocumentLinkService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseDisposalDocumentLinkService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseDisposalDocumentLinkService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseDisposalDocumentLinkService.CacheData.Data, this.endOfLeaseDisposalDocumentLinkService.CacheData.TotalRecords);
    }
  }
