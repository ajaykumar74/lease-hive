...
@Component({
  ...
  standalone: false,
  templateUrl: './disposalCase-list.component.html'
})
export class DisposalCaseListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.disposalCaseService.CacheData.IsLoaded) {
      this.currentPage = this.disposalCaseService.CacheData.CurrentPage;
      this.objSearch = this.disposalCaseService.CacheData.objSearch;
      this.permission = this.disposalCaseService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.disposalCaseService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.disposalCaseService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.disposalCaseService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.disposalCaseService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.disposalCaseService.CacheData.Data, this.disposalCaseService.CacheData.TotalRecords);
    }
  }
