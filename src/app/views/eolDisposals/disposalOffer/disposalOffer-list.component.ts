...
@Component({
  ...
  standalone: false,
  templateUrl: './disposalOffer-list.component.html'
})
export class DisposalOfferListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.disposalOfferService.CacheData.IsLoaded) {
      this.currentPage = this.disposalOfferService.CacheData.CurrentPage;
      this.objSearch = this.disposalOfferService.CacheData.objSearch;
      this.permission = this.disposalOfferService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.disposalOfferService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.disposalOfferService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.disposalOfferService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.disposalOfferService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.disposalOfferService.CacheData.Data, this.disposalOfferService.CacheData.TotalRecords);
    }
  }
