...
@Component({
  ...
  standalone: false,
  templateUrl: './returnAssessmentDamage-list.component.html'
})
export class ReturnAssessmentDamageListComponent implements OnInit {
  ...
  lstMain: IReturnAssessmentDamage[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.returnAssessmentDamageService.CacheData.IsLoaded) {
      this.currentPage = this.returnAssessmentDamageService.CacheData.CurrentPage;
      this.objSearch = this.returnAssessmentDamageService.CacheData.objSearch;
      this.permission = this.returnAssessmentDamageService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.returnAssessmentDamageService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.returnAssessmentDamageService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.returnAssessmentDamageService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.returnAssessmentDamageService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.returnAssessmentDamageService.CacheData.Data, this.returnAssessmentDamageService.CacheData.TotalRecords);
    }
  }
