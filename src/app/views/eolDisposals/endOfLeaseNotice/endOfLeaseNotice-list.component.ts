...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseNotice-list.component.html'
})
export class EndOfLeaseNoticeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.endOfLeaseNoticeService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseNoticeService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseNoticeService.CacheData.objSearch;
      this.permission = this.endOfLeaseNoticeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseNoticeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseNoticeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseNoticeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseNoticeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseNoticeService.CacheData.Data, this.endOfLeaseNoticeService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/endOfLeaseNotices/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/endOfLeaseNotices/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/endOfLeaseNotices/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
