...
@Component({
  ...
  standalone: false,
  templateUrl: './assetReturn-list.component.html'
})
export class AssetReturnListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetReturnService.CacheData.IsLoaded) {
      this.currentPage = this.assetReturnService.CacheData.CurrentPage;
      this.objSearch = this.assetReturnService.CacheData.objSearch;
      this.permission = this.assetReturnService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetReturnService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetReturnService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetReturnService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetReturnService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetReturnService.CacheData.Data, this.assetReturnService.CacheData.TotalRecords);
    }
  }
