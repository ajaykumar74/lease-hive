...
@Component({
  ...
  standalone: false,
  templateUrl: './assetScrap-list.component.html'
})
export class AssetScrapListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetScrapService.CacheData.IsLoaded) {
      this.currentPage = this.assetScrapService.CacheData.CurrentPage;
      this.objSearch = this.assetScrapService.CacheData.objSearch;
      this.permission = this.assetScrapService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetScrapService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetScrapService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetScrapService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetScrapService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetScrapService.CacheData.Data, this.assetScrapService.CacheData.TotalRecords);
    }
  }
