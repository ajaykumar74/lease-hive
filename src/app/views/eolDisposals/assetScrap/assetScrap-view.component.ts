...
@Component({
    templateUrl: './assetScrap-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetScrapViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetScrap: IAssetScrap = {} as IAssetScrap;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetScrap/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
