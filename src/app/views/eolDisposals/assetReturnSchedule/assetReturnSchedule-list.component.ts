...
@Component({
  ...
  standalone: false,
  templateUrl: './assetReturnSchedule-list.component.html'
})
export class AssetReturnScheduleListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetReturnScheduleService.CacheData.IsLoaded) {
      this.currentPage = this.assetReturnScheduleService.CacheData.CurrentPage;
      this.objSearch = this.assetReturnScheduleService.CacheData.objSearch;
      this.permission = this.assetReturnScheduleService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetReturnScheduleService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetReturnScheduleService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetReturnScheduleService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetReturnScheduleService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetReturnScheduleService.CacheData.Data, this.assetReturnScheduleService.CacheData.TotalRecords);
    }
  }
