...
@Component({
    templateUrl: './assetReturnSchedule-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetReturnScheduleViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetReturnSchedule: IAssetReturnSchedule = {} as IAssetReturnSchedule;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetReturnSchedule/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
