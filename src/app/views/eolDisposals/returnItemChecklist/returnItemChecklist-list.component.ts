...
@Component({
  ...
  standalone: false,
  templateUrl: './returnItemChecklist-list.component.html'
})
export class ReturnItemChecklistListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.returnItemChecklistService.CacheData.IsLoaded) {
      this.currentPage = this.returnItemChecklistService.CacheData.CurrentPage;
      this.objSearch = this.returnItemChecklistService.CacheData.objSearch;
      this.permission = this.returnItemChecklistService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.returnItemChecklistService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.returnItemChecklistService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.returnItemChecklistService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.returnItemChecklistService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.returnItemChecklistService.CacheData.Data, this.returnItemChecklistService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/eol-disposal/returns/checklists/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/eol-disposal/returns/checklists/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/eol-disposal/returns/checklists/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
