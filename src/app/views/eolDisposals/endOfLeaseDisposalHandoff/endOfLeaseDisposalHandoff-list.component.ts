...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseDisposalHandoff-list.component.html'
})
export class EndOfLeaseDisposalHandoffListComponent implements OnInit {
  ...
  lstMain: IEndOfLeaseDisposalHandoff[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.endOfLeaseDisposalHandoffService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseDisposalHandoffService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseDisposalHandoffService.CacheData.objSearch;
      this.permission = this.endOfLeaseDisposalHandoffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseDisposalHandoffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseDisposalHandoffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseDisposalHandoffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseDisposalHandoffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseDisposalHandoffService.CacheData.Data, this.endOfLeaseDisposalHandoffService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/endOfLeaseDisposalHandoffs/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/endOfLeaseDisposalHandoffs/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/endOfLeaseDisposalHandoffs/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
