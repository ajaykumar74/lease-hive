...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseDisposalException-list.component.html'
})
export class EndOfLeaseDisposalExceptionListComponent implements OnInit {
  ...
  lstMain: IEndOfLeaseDisposalException[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.endOfLeaseDisposalExceptionService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseDisposalExceptionService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseDisposalExceptionService.CacheData.objSearch;
      this.permission = this.endOfLeaseDisposalExceptionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseDisposalExceptionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseDisposalExceptionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseDisposalExceptionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseDisposalExceptionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseDisposalExceptionService.CacheData.Data, this.endOfLeaseDisposalExceptionService.CacheData.TotalRecords);
    }
  }
