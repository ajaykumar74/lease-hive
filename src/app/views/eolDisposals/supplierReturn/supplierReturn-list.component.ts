...
export class SupplierReturnListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.supplierReturnService.CacheData.IsLoaded) {
      this.currentPage = this.supplierReturnService.CacheData.CurrentPage;
      this.objSearch = this.supplierReturnService.CacheData.objSearch;
      this.permission = this.supplierReturnService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.supplierReturnService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.supplierReturnService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.supplierReturnService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.supplierReturnService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.supplierReturnService.CacheData.Data, this.supplierReturnService.CacheData.TotalRecords);
    }
  }
