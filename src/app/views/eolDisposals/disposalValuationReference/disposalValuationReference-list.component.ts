...
@Component({
  ...
  standalone: false,
  templateUrl: './disposalValuationReference-list.component.html'
})
export class DisposalValuationReferenceListComponent implements OnInit {
  ...
  lstMain: IDisposalValuationReference[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.disposalValuationReferenceService.CacheData.IsLoaded) {
      this.currentPage = this.disposalValuationReferenceService.CacheData.CurrentPage;
      this.objSearch = this.disposalValuationReferenceService.CacheData.objSearch;
      this.permission = this.disposalValuationReferenceService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.disposalValuationReferenceService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.disposalValuationReferenceService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.disposalValuationReferenceService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.disposalValuationReferenceService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.disposalValuationReferenceService.CacheData.Data, this.disposalValuationReferenceService.CacheData.TotalRecords);
    }
  }
