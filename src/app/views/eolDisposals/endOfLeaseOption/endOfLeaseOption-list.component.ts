...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseOption-list.component.html'
})
export class EndOfLeaseOptionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.endOfLeaseOptionService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseOptionService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseOptionService.CacheData.objSearch;
      this.permission = this.endOfLeaseOptionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseOptionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseOptionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseOptionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseOptionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseOptionService.CacheData.Data, this.endOfLeaseOptionService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/endOfLeaseOptions/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/endOfLeaseOptions/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/endOfLeaseOptions/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
