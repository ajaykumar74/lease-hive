...
@Component({
  ...
  standalone: false,
  templateUrl: './purchaseOptionExercise-list.component.html'
})
export class PurchaseOptionExerciseListComponent implements OnInit {
  ...
  lstMain: IPurchaseOptionExercise[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.purchaseOptionExerciseService.CacheData.IsLoaded) {
      this.currentPage = this.purchaseOptionExerciseService.CacheData.CurrentPage;
      this.objSearch = this.purchaseOptionExerciseService.CacheData.objSearch;
      this.permission = this.purchaseOptionExerciseService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.purchaseOptionExerciseService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.purchaseOptionExerciseService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.purchaseOptionExerciseService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.purchaseOptionExerciseService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.purchaseOptionExerciseService.CacheData.Data, this.purchaseOptionExerciseService.CacheData.TotalRecords);
    }
  }
