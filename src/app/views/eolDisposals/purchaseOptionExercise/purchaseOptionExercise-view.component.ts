...
@Component({
    templateUrl: './purchaseOptionExercise-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class PurchaseOptionExerciseViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    purchaseOptionExercise: IPurchaseOptionExercise = {} as IPurchaseOptionExercise;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/purchaseOptionExercise/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
