...
@Component({
  ...
  standalone: false,
  templateUrl: './assetWriteOff-list.component.html'
})
export class AssetWriteOffListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetWriteOffService.CacheData.IsLoaded) {
      this.currentPage = this.assetWriteOffService.CacheData.CurrentPage;
      this.objSearch = this.assetWriteOffService.CacheData.objSearch;
      this.permission = this.assetWriteOffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetWriteOffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetWriteOffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetWriteOffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetWriteOffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetWriteOffService.CacheData.Data, this.assetWriteOffService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/eol-disposal/disposition/write-offs/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/eol-disposal/disposition/write-offs/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/eol-disposal/disposition/write-offs/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
