...
@Component({
  ...
  standalone: false,
  templateUrl: './refurbishmentHandoff-list.component.html'
})
export class RefurbishmentHandoffListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.refurbishmentHandoffService.CacheData.IsLoaded) {
      this.currentPage = this.refurbishmentHandoffService.CacheData.CurrentPage;
      this.objSearch = this.refurbishmentHandoffService.CacheData.objSearch;
      this.permission = this.refurbishmentHandoffService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.refurbishmentHandoffService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.refurbishmentHandoffService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.refurbishmentHandoffService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.refurbishmentHandoffService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.refurbishmentHandoffService.CacheData.Data, this.refurbishmentHandoffService.CacheData.TotalRecords);
    }
  }
