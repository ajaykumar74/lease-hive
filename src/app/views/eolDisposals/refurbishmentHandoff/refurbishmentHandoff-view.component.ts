...
@Component({
    templateUrl: './refurbishmentHandoff-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class RefurbishmentHandoffViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    refurbishmentHandoff: IRefurbishmentHandoff = {} as IRefurbishmentHandoff;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/refurbishmentHandoff/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
