...
@Component({
  ...
  standalone: false,
  templateUrl: './excessUsageAssessment-list.component.html'
})
export class ExcessUsageAssessmentListComponent implements OnInit {
  ...
  lstMain: IExcessUsageAssessment[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.excessUsageAssessmentService.CacheData.IsLoaded) {
      this.currentPage = this.excessUsageAssessmentService.CacheData.CurrentPage;
      this.objSearch = this.excessUsageAssessmentService.CacheData.objSearch;
      this.permission = this.excessUsageAssessmentService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.excessUsageAssessmentService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.excessUsageAssessmentService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.excessUsageAssessmentService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.excessUsageAssessmentService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.excessUsageAssessmentService.CacheData.Data, this.excessUsageAssessmentService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/excessUsageAssessments/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/excessUsageAssessments/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/excessUsageAssessments/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
