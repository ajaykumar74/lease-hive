...
@Component({
  ...
  standalone: false,
  templateUrl: './endOfLeaseSettlementLine-list.component.html'
})
export class EndOfLeaseSettlementLineListComponent implements OnInit {
  ...
  lstMain: IEndOfLeaseSettlementLine[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.endOfLeaseSettlementLineService.CacheData.IsLoaded) {
      this.currentPage = this.endOfLeaseSettlementLineService.CacheData.CurrentPage;
      this.objSearch = this.endOfLeaseSettlementLineService.CacheData.objSearch;
      this.permission = this.endOfLeaseSettlementLineService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.endOfLeaseSettlementLineService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.endOfLeaseSettlementLineService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.endOfLeaseSettlementLineService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.endOfLeaseSettlementLineService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.endOfLeaseSettlementLineService.CacheData.Data, this.endOfLeaseSettlementLineService.CacheData.TotalRecords);
    }
  }
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/endOfLeaseSettlementLines/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/endOfLeaseSettlementLines/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/endOfLeaseSettlementLines/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
