...
export class CustomerProfileListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.customerProfileService.CacheData.IsLoaded) {
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.customerProfileService.CacheData.IsLoaded);
    }, 500);
  }
    ...
    if (isReload || this.customerProfileService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.customerProfileService.search(searchParam).subscribe({
        next: res => {
          ...
          this.customerProfileService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/customerProfiles/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/customerProfiles/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/customerProfiles/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
