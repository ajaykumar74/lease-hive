...
@Component({
  ...
  standalone: false,
  templateUrl: './assetAcquisition-list.component.html'
})
export class AssetAcquisitionListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetAcquisitionService.CacheData.IsLoaded) {
      this.currentPage = this.assetAcquisitionService.CacheData.CurrentPage;
      this.objSearch = this.assetAcquisitionService.CacheData.objSearch;
      this.permission = this.assetAcquisitionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetAcquisitionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetAcquisitionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetAcquisitionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetAcquisitionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetAcquisitionService.CacheData.Data, this.assetAcquisitionService.CacheData.TotalRecords);
    }
  }
