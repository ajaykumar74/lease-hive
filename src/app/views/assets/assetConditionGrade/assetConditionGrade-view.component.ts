...
@Component({
    templateUrl: './assetConditionGrade-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetConditionGradeViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetConditionGrade: IAssetConditionGrade = {} as IAssetConditionGrade;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetConditionGrade/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
