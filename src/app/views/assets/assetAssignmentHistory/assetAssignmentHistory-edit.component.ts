...
@Component({
  selector: 'app-assetAssignmentHistory-edit',
  standalone: false,
  templateUrl: './assetAssignmentHistory-edit.component.html',
...
Remarks: obj.Remarks || '',
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/assetAssignmentHistory/create', { id: -1 }]);
    }
    else if (key == "Save") {
      this.Save();
    }
...
FromAssetUserId:  formValues.FromAssetUserId || null,
ToAssetUserId:  formValues.ToAssetUserId || null,
FromPartyLocationId:  formValues.FromPartyLocationId || null,
ToPartyLocationId:  formValues.ToPartyLocationId || null,
Remarks:  formValues.Remarks || null,

    } as IAssetAssignmentHistory ;
	
	this.spinner.show();  	   
    this.assetAssignmentHistoryService.update(this.assetAssignmentHistory.Id, updatedObj).subscribe({
      next: data => {
