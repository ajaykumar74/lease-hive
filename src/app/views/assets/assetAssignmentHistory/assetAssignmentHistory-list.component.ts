...
@Component({
  ...
  standalone: false,
  templateUrl: './assetAssignmentHistory-list.component.html'
})
export class AssetAssignmentHistoryListComponent implements OnInit {
  ...
  lstMain: IAssetAssignmentHistory[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.assetAssignmentHistoryService.CacheData.IsLoaded) {
      this.currentPage = this.assetAssignmentHistoryService.CacheData.CurrentPage;
      this.objSearch = this.assetAssignmentHistoryService.CacheData.objSearch;
      this.permission = this.assetAssignmentHistoryService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetAssignmentHistoryService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetAssignmentHistoryService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetAssignmentHistoryService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetAssignmentHistoryService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetAssignmentHistoryService.CacheData.Data, this.assetAssignmentHistoryService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'EventTypeId', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
