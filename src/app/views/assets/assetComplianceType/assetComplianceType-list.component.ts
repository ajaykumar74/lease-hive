...
@Component({
  ...
  standalone: false,
  templateUrl: './assetComplianceType-list.component.html'
})
export class AssetComplianceTypeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetComplianceTypeService.CacheData.IsLoaded) {
      this.currentPage = this.assetComplianceTypeService.CacheData.CurrentPage;
      this.objSearch = this.assetComplianceTypeService.CacheData.objSearch;
      this.permission = this.assetComplianceTypeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetComplianceTypeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetComplianceTypeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetComplianceTypeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetComplianceTypeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetComplianceTypeService.CacheData.Data, this.assetComplianceTypeService.CacheData.TotalRecords);
    }
  }
