...
@Component({
  ...
  standalone: false,
  templateUrl: './assetLocationHistory-list.component.html'
})
export class AssetLocationHistoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.assetId = routeAssetId > 0 ? routeAssetId : null;
     if (this.isCurrentContextCached()) {
      this.currentPage = this.assetLocationHistoryService.CacheData.CurrentPage;
      this.objSearch = this.assetLocationHistoryService.CacheData.objSearch;
      this.permission = this.assetLocationHistoryService.CacheData.permission;
    }  
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetLocationHistoryService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetLocationHistoryService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetLocationHistoryService.setCache(res.data, this.permission, this.objSearch, pgEvent.page, this.contextKey);
        },
      ...
      this.SetListData(this.assetLocationHistoryService.CacheData.Data, this.assetLocationHistoryService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'MovementType', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onViewClick(obj: any): void {
    const route = this.assetId
      ? ['/business/assets/location-history/asset', this.assetId, 'view', obj.Id]
      : ['/business/assets/location-history/view', obj.Id];
  ...
  onDetailsClick(obj: any): void {
    ...
    const route = this.assetId
      ? ['/business/assets/location-history/asset', this.assetId, page, obj.Id]
      : ['/business/assets/location-history', page, obj.Id];
    this.router.navigate(route);
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      const route = this.assetId
        ? ['/business/assets/location-history/asset', this.assetId, 'create']
        : ['/business/assets/location-history/create'];
      this.router.navigate(route);
    } 
    else if (key == "Refresh") {
      this.search();
  ...
  private isCurrentContextCached(): boolean {
    return !!this.assetLocationHistoryService.CacheData.IsLoaded
      && this.assetLocationHistoryService.CacheContextKey === this.contextKey;
  }
}
