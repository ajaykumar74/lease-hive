...
export class AssetTypeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetTypeService.CacheData.IsLoaded) {
      this.currentPage = this.assetTypeService.CacheData.CurrentPage;
      this.objSearch = this.assetTypeService.CacheData.objSearch;
      this.permission = this.assetTypeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetTypeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetTypeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetTypeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetTypeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetTypeService.CacheData.Data, this.assetTypeService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'AssetTypeName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'AssetTypeCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['/business/assets/classification/types/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['/business/assets/classification/types/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/business/assets/classification/types/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
