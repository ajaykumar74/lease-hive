...
@Component({
    templateUrl: './assetMeasureDefinition-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetMeasureDefinitionViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetMeasureDefinition: IAssetMeasureDefinition = {} as IAssetMeasureDefinition;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetMeasureDefinition/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
