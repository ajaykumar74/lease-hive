...
@Component({
  ...
  standalone: false,
  templateUrl: './assetMeasureDefinition-list.component.html'
})
export class AssetMeasureDefinitionListComponent implements OnInit {
  ...
  lstMain: IAssetMeasureDefinition[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.assetMeasureDefinitionService.CacheData.IsLoaded) {
      this.currentPage = this.assetMeasureDefinitionService.CacheData.CurrentPage;
      this.objSearch = this.assetMeasureDefinitionService.CacheData.objSearch;
      this.permission = this.assetMeasureDefinitionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetMeasureDefinitionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetMeasureDefinitionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetMeasureDefinitionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetMeasureDefinitionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetMeasureDefinitionService.CacheData.Data, this.assetMeasureDefinitionService.CacheData.TotalRecords);
    }
  }
