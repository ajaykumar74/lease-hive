...
@Component({
  ...
  standalone: false,
  templateUrl: './assetAttributeDefinition-list.component.html'
})
export class AssetAttributeDefinitionListComponent implements OnInit {
  ...
  lstMain: IAssetAttributeDefinition[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.assetAttributeDefinitionService.CacheData.IsLoaded) {
      this.currentPage = this.assetAttributeDefinitionService.CacheData.CurrentPage;
      this.objSearch = this.assetAttributeDefinitionService.CacheData.objSearch;
      this.permission = this.assetAttributeDefinitionService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetAttributeDefinitionService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetAttributeDefinitionService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetAttributeDefinitionService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetAttributeDefinitionService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetAttributeDefinitionService.CacheData.Data, this.assetAttributeDefinitionService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'AttributeName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'AttributeCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
