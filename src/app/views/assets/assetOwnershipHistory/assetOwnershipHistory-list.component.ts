...
@Component({
  ...
  standalone: false,
  templateUrl: './assetOwnershipHistory-list.component.html'
})
export class AssetOwnershipHistoryListComponent implements OnInit {
  ...
  lstMain: IAssetOwnershipHistory[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.assetOwnershipHistoryService.CacheData.IsLoaded) {
      this.currentPage = this.assetOwnershipHistoryService.CacheData.CurrentPage;
      this.objSearch = this.assetOwnershipHistoryService.CacheData.objSearch;
      this.permission = this.assetOwnershipHistoryService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetOwnershipHistoryService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetOwnershipHistoryService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetOwnershipHistoryService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetOwnershipHistoryService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetOwnershipHistoryService.CacheData.Data, this.assetOwnershipHistoryService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'OwnershipType', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
  ...
  onDetailsClick(obj: any): void {
    if (this.permission.CanCreate || this.permission.CanUpdate) {
        this.router.navigate(['dashboard/assetOwnershipHistorys/edit/' + obj.Id]);
    }
    else {
        this.router.navigate(['dashboard/assetOwnershipHistorys/view/' + obj.Id]);
    } 
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['dashboard/assetOwnershipHistorys/create']);
    } 
    else if (key == "Refresh") {
      this.search();
    }
