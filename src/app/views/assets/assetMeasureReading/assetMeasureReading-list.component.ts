...
@Component({
  ...
  standalone: false,
  templateUrl: './assetMeasureReading-list.component.html'
})
export class AssetMeasureReadingListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetMeasureReadingService.CacheData.IsLoaded) {
      this.currentPage = this.assetMeasureReadingService.CacheData.CurrentPage;
      this.objSearch = this.assetMeasureReadingService.CacheData.objSearch;
      this.permission = this.assetMeasureReadingService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetMeasureReadingService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetMeasureReadingService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetMeasureReadingService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetMeasureReadingService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetMeasureReadingService.CacheData.Data, this.assetMeasureReadingService.CacheData.TotalRecords);
    }
  }
