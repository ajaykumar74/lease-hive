...
@Component({
    templateUrl: './assetMeasureReading-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetMeasureReadingViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetMeasureReading: IAssetMeasureReading = {} as IAssetMeasureReading;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetMeasureReading/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
