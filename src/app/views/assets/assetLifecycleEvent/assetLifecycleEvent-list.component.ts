...
@Component({
  ...
  standalone: false,
  templateUrl: './assetLifecycleEvent-list.component.html'
})
export class AssetLifecycleEventListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetLifecycleEventService.CacheData.IsLoaded) {
      this.currentPage = this.assetLifecycleEventService.CacheData.CurrentPage;
      this.objSearch = this.assetLifecycleEventService.CacheData.objSearch;
      this.permission = this.assetLifecycleEventService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetLifecycleEventService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetLifecycleEventService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetLifecycleEventService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetLifecycleEventService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetLifecycleEventService.CacheData.Data, this.assetLifecycleEventService.CacheData.TotalRecords);
    }
  }
