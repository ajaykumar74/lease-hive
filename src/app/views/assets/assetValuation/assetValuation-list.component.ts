...
@Component({
  ...
  standalone: false,
  templateUrl: './assetValuation-list.component.html'
})
export class AssetValuationListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetValuationService.CacheData.IsLoaded) {
      this.currentPage = this.assetValuationService.CacheData.CurrentPage;
      this.objSearch = this.assetValuationService.CacheData.objSearch;
      this.permission = this.assetValuationService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetValuationService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetValuationService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetValuationService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetValuationService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetValuationService.CacheData.Data, this.assetValuationService.CacheData.TotalRecords);
    }
  }
