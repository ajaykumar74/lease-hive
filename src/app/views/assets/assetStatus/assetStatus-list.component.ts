...
import { IAssetStatus } from './assetStatus';
import { PageEvent } from '@/shared/IBase';

@Component({
  selector: 'app-customer-list',
  standalone: false,
  templateUrl: './assetStatus-list.component.html'
})
export class AssetStatusListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetStatusService.CacheData.IsLoaded) {
      this.currentPage = this.assetStatusService.CacheData.CurrentPage;
      this.objSearch = this.assetStatusService.CacheData.objSearch;
      this.permission = this.assetStatusService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetStatusService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetStatusService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetStatusService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetStatusService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetStatusService.CacheData.Data, this.assetStatusService.CacheData.TotalRecords);
    }
  }
