...
@Component({
    templateUrl: './vehicleAsset-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class VehicleAssetViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    vehicleAsset: IVehicleAsset = {} as IVehicleAsset;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/vehicleAsset/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
