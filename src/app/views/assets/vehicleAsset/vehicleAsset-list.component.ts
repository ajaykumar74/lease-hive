...
export class VehicleAssetListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.vehicleAssetService.CacheData.IsLoaded) {
      this.currentPage = this.vehicleAssetService.CacheData.CurrentPage;
      this.objSearch = this.vehicleAssetService.CacheData.objSearch;
      this.permission = this.vehicleAssetService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.vehicleAssetService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.vehicleAssetService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.vehicleAssetService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.vehicleAssetService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.vehicleAssetService.CacheData.Data, this.vehicleAssetService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'RegistrationNo', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
