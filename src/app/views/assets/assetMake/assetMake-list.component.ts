...
@Component({
  ...
  standalone: false,
  templateUrl: './assetMake-list.component.html'
})
export class AssetMakeListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetMakeService.CacheData.IsLoaded) {
      this.currentPage = this.assetMakeService.CacheData.CurrentPage;
      this.objSearch = this.assetMakeService.CacheData.objSearch;
      this.permission = this.assetMakeService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetMakeService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetMakeService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetMakeService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetMakeService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetMakeService.CacheData.Data, this.assetMakeService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'MakeCode', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'MakeName', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
