...
@Component({
  ...
  standalone: false,
  templateUrl: './iTAsset-list.component.html'
})
export class ITAssetListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.iTAssetService.CacheData.IsLoaded) {
      this.currentPage = this.iTAssetService.CacheData.CurrentPage;
      this.objSearch = this.iTAssetService.CacheData.objSearch;
      this.permission = this.iTAssetService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.iTAssetService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.iTAssetService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.iTAssetService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.iTAssetService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.iTAssetService.CacheData.Data, this.iTAssetService.CacheData.TotalRecords);
    }
  }
