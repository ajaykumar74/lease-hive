...
@Component({
    templateUrl: './assetAttributeOption-view.component.html', 
standalone: false,
    providers: [MessageService]
})
export class AssetAttributeOptionViewComponent implements OnInit {
    selectedId: number;
    isLoading: boolean = false;
    permission = { CanCreate: true } as IPermission;
    assetAttributeOption: IAssetAttributeOption = {} as IAssetAttributeOption;
    ...
    onOptionItemClicked(key: string): void {
        if (key == "Refresh") {             
            this.router.navigate(['/assetAttributeOption/create']);
        }        
        else if (key == "Refresh") {
            this.loadUI();
        }
