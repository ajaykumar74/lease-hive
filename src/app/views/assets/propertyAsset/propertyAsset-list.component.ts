...
@Component({
  ...
  standalone: false,
  templateUrl: './propertyAsset-list.component.html'
})
export class PropertyAssetListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.propertyAssetService.CacheData.IsLoaded) {
      this.currentPage = this.propertyAssetService.CacheData.CurrentPage;
      this.objSearch = this.propertyAssetService.CacheData.objSearch;
      this.permission = this.propertyAssetService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.propertyAssetService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.propertyAssetService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.propertyAssetService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.propertyAssetService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.propertyAssetService.CacheData.Data, this.propertyAssetService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    Items = [
      ...
      { DBName: 'PropertyType', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
