...
@Component({
  ...
  standalone: false,
  templateUrl: './assetComplianceRecord-list.component.html'
})
export class AssetComplianceRecordListComponent implements OnInit {
  ...
  lstMain: IAssetComplianceRecord[]; 
  sortBy: string = 'Id';
  ...
  ngOnInit(): void {
     if (this.assetComplianceRecordService.CacheData.IsLoaded) {
      this.currentPage = this.assetComplianceRecordService.CacheData.CurrentPage;
      this.objSearch = this.assetComplianceRecordService.CacheData.objSearch;
      this.permission = this.assetComplianceRecordService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetComplianceRecordService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetComplianceRecordService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetComplianceRecordService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetComplianceRecordService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetComplianceRecordService.CacheData.Data, this.assetComplianceRecordService.CacheData.TotalRecords);
    }
  }
