...
@Component({
  selector: 'app-assetComplianceRecord-create',
  standalone: false,
  templateUrl: './assetComplianceRecord-create.component.html' ,
   providers: [ MessageService]
})
export class AssetComplianceRecordCreateComponent implements OnInit {
...
VerifiedOn: new FormControl(new Date(), []),
    ...
    this.Caption = 'Create AssetComplianceRecord';
...
this.loggedInUserService.bindEntityLookup(this.editForm, 'VerifiedBy', 'application-users', options => this.verifiedbyOptions = options, error => this.messageService.showError(error), this.entityLookupDestroyRef);
 ...
 loadUI(): void {
    ...
    this.assetComplianceRecordService.getById(this.selectedId).subscribe({
      next: data => {
        this.assetComplianceRecord = data;
        this.objMaster = { ...this.assetComplianceRecord };
...
AssetComplianceTypeId: formValues.AssetComplianceTypeId || 0,
CertificateNo: formValues.CertificateNo || null,
IssuedByPartyId: formValues.IssuedByPartyId || 0,
...
VerificationStatusId: formValues.VerificationStatusId || 0,
VerifiedBy: formValues.VerifiedBy || 0,
VerifiedOn: formValues.VerifiedOn || null,

    } as IAssetComplianceRecord ; 
	applyAssetPayloadDefaults(createdObj, formValues, ['AssetId', 'AssetComplianceTypeId', 'IssuedByPartyId', 'DocumentId', 'VerificationStatusId', 'VerifiedBy'], [], ['IssueDate', 'ValidFrom', 'ValidTo', 'VerifiedOn']);
	
	  this.spinner.show(); 
    this.assetComplianceRecordService.create(createdObj).subscribe({
      next: data => {	   
