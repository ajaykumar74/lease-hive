...
@Component({
  ...
  standalone: false,
  templateUrl: './assetModel-list.component.html'
})
export class AssetModelListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetModelService.CacheData.IsLoaded) {
      this.currentPage = this.assetModelService.CacheData.CurrentPage;
      this.objSearch = this.assetModelService.CacheData.objSearch;
      this.permission = this.assetModelService.CacheData.permission;
    }  
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetModelService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetModelService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetModelService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetModelService.CacheData.Data, this.assetModelService.CacheData.TotalRecords);
    }
  }
  ...
  getSearchParams() {
    ...
    //  { DBName: 'OperatorId', Value: '', DataType: DataType.Int, Operator: Operator.EqualTo },
      { DBName: 'ModelName', Value: this.objSearch.Name, DataType: DataType.Text, Operator: Operator.Contains },
      { DBName: 'ModelCode', Value: this.objSearch.Code, DataType: DataType.Text, Operator: Operator.Contains },
    ];
