...
@Component({
  ...
  standalone: false,
  templateUrl: './assetDocumentLink-list.component.html'
})
export class AssetDocumentLinkListComponent implements OnInit {
  ...
  ngOnInit(): void {
     if (this.assetDocumentLinkService.CacheData.IsLoaded) {
      this.currentPage = this.assetDocumentLinkService.CacheData.CurrentPage;
      this.objSearch = this.assetDocumentLinkService.CacheData.objSearch;
      this.permission = this.assetDocumentLinkService.CacheData.permission;
    }  
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.searchData(this.pgEvent, !this.assetDocumentLinkService.CacheData.IsLoaded);
    }, 500);
  }
  ...
  pageChanged(arg): void {
    ...
    if (isReload || this.assetDocumentLinkService.CacheData.CurrentPage != pgEvent.page) {
      ...
      this.isLoading = true;
      this.assetDocumentLinkService.search(searchParam).subscribe({
        next: res => {
          this.permission = res.permission; 
          this.SetListData(res.data.Records, res.data.TotalRecords);
          this.assetDocumentLinkService.setCache(res.data, this.permission, this.objSearch, pgEvent.page);
        },
      ...
      this.SetListData(this.assetDocumentLinkService.CacheData.Data, this.assetDocumentLinkService.CacheData.TotalRecords);
    }
  }
